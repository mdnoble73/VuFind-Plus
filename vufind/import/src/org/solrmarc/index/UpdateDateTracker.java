...
public class UpdateDateTracker {
	...
	private void createRow(Timestamp newRecordChange) throws SQLException {
		...
		PreparedStatement sql = db.prepareStatement("INSERT INTO change_tracker(core, id, first_indexed, last_indexed, last_record_change) "
				+ "VALUES(?, ?, ?, ?, ?);");
	...
	private boolean readRow() throws SQLException {
		PreparedStatement sql = db.prepareStatement("SELECT first_indexed, last_indexed, last_record_change, deleted "
	...
	private void updateRow(Timestamp newRecordChange) throws SQLException {
		...
		// Save new values to the database:
		PreparedStatement sql = db.prepareStatement("UPDATE change_tracker " + "SET first_indexed = ?, last_indexed = ?, last_record_change = ?, deleted = ? "
				+ "WHERE core = ? AND id = ?;");
	...
	/*
	 * Get the first indexed date (IMPORTANT: index() must be called before this
	 * method)
	 */
	...
	/*
	 * Get the last indexed date (IMPORTANT: index() must be called before this
	 * method)
	 */
	...
	/*
	 * Update the database to indicate that the record has just been received by
	 * the indexer:
	 */
	public void index(String selectedCore, String selectedId, java.util.Date recordChange) throws SQLException {
		...
		if (!readRow()) {
			...
			// false positives.
			if (deleted != null || Math.abs(lastRecordChange.getTime() - newRecordChange.getTime()) > 999) {
