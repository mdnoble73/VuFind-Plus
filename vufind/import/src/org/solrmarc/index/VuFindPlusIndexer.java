 ...
 * 
 */
public class VuFindPlusIndexer extends SolrIndexer {
	// Initialize logging category
	static Logger										logger					= Logger.getLogger(VuFindPlusIndexer.class.getName());

	// Initialize VuFind database connection (null until explicitly activated)
	private Connection							vufindDatabase		= null;
	private Connection							econtentDatabase	= null;
	private UpdateDateTracker				tracker						= null;
	
	private CheckSuppression				suppressionChecker = null;
	...
	private void connectToDatabases() {
		...
		try {
			...
			vufindDatabase = DriverManager.getConnection(dsn);
			System.out.println("Connected to VuFind Database");
		} catch (Throwable e) {
			dieWithError("Unable to connect to VuFind database");
		}
		
		String econtentDsn = Util.cleanIniValue(ini.get("Database", "database_econtent_jdbc"));
		try {
			// Connect to the database:
			econtentDatabase = DriverManager.getConnection(econtentDsn);
			System.out.println("Connected to eContent Database");
		} catch (Throwable e) {
			dieWithError("Unable to connect to eContent database");
		}
	...
	private void disconnectFromDatabase() {
		...
		if (econtentDatabase != null) {
			try {
				econtentDatabase.close();
			} catch (SQLException e) {
				System.err.println("Unable to disconnect from eContent database");
				logger.error("Unable to disconnect from eContent database");
			}
		}
	}
	...
	class VuFindShutdownThread extends Thread {
		private VuFindPlusIndexer	indexer;

		public VuFindShutdownThread(VuFindPlusIndexer i) {
			indexer = i;
		}
	...
	private java.util.Date normalize005Date(String input) {
		...
		// Try to parse the date; default to "millisecond 0" (very old date) if we
		// can't
	...
	public java.util.Date getLatestTransaction(Record record) {
		// First try the 005 -- this is most likely to have a precise transaction
		// date:
		...
		// If we got this far, we couldn't find a valid value; return an arbitrary
		// date:
	 ...
	 *          record
	 * @param String
	 *          field spec to search for URLs
	 * @param String
	 *          only harvest files matching this extension (null for all)
	 * @return String The full-text
	 */
	public String getFulltext(Record record, String fieldSpec, String extension) {
		String result = "";

		// Get the path to Aperture web crawler (and return no text if it is
		// unavailable)
	 ...
	 * web/conf/fulltext.ini file. Without proper configuration, this will simply
	 * return an empty string.
	 * 
	 * @param String
	 *          The url extracted from the MARC tag.
	 * @param String
	 *          The path to Aperture
	 * @return String The full-text
	 */
	...
	public String checkSuppression(Record record, String recordIdSpec, String suppressionFieldSpec, String suppressionPattern){
		if (suppressionChecker == null){
