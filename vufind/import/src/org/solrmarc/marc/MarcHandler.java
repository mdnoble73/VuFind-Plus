...
public abstract class MarcHandler {
	...
	public void loadProperties(String configProperties) {
		...
		if (combineConsecutiveRecordsFields != null && combineConsecutiveRecordsFields.length() == 0) combineConsecutiveRecordsFields = null;
		...
		} else // no override, tell solrmarc to use the NoSortMarcFactory by
						// default.
		{
	...
	// We only get here if the parm (unicodeNormalize2) is not null compare it
	// against
	...
	/**
	 * normalizePathsProperty - normalize one or more : separated paths using the
	 * normalizePathProperty method
	 * 
	 * @param homeDir
	 ...
	 * "current" directory (which is whatever directory the user happens to be in
	 * when the the JVM was started. Also resolve the property strings
	...
	public void loadReader(String source, String fName) {
		if (source.equals("FILE") || source.equals("STDIN")) {
			...
			if (source.equals("FILE")) {
				if (fName != null && fName.toLowerCase().endsWith(".xml"))
					inputTypeXML = true;
				else if (fName != null && fName.toLowerCase().endsWith(".json")) inputTypeJSON = true;
				try {
					if (showInputFile)
		...
		// Do translating last so that if we are Filtering as well as translating,
		// we don't expend the
