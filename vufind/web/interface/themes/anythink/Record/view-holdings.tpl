...
{if isset($holdings) && count($holdings) > 0}
 ...
 {foreach from=$holdings item=holding}
  ...
  <tr >
    ...
    <td  style = "padding-bottom:5px;">
    	{if $holding.luckyDay}
    	{assign var="showLuckyDayNote" value="true"}
    	<img src='{$path}/interface/themes/dcl/images/yld.png' alt='Lucky Day' title='This item cannot have holds placed on it and cannot be renewed.'/>
    	{/if}
    </td>
  </tr>
...
{/if}
{if $showLuckyDayNote}
	<div id="luckyDayNote">Items marked with a <img src='{$path}/interface/themes/dcl/images/yld.png' alt='Lucky Day' title=''/> cannot have holds placed on them and cannot be renewed.</div>
{/if}
