  ...
  <div id="main-content" class="full-result-content">
    <div id="record-header">
      ...
      <div id ="recordTitleRight">
		    {if isset($nextId)}
		      <div id="nextRecordLink"><a href="{$path}/{$nextType}/{$nextId|escape:"url"}?searchId={$searchId}&amp;recordIndex={$nextIndex}&amp;page={if isset($nextPage)}{$nextPage}{else}{$page}{/if}" title="{if !$nextTitle}{translate text='Next'}{else}{$nextTitle|truncate:180:"..."}{/if}"><img src="{$path}/interface/themes/anythink/images/next.png" alt="Next Record"/></a></div>
		    {/if}
      	{if $lastsearch}
      ...
      <div id="myrating" class="stat">
        ...
        $(
         function() {literal} { {/literal}
             $('#myrating').rater({literal}{ {/literal} rating:'{$ratingData.average}', recordId: '{$id}', postHref: '{$path}/Record/{$id}/AJAX?method=RateTitle'{literal} } {/literal});
         {literal} } {/literal}
	    );
        </script>
      </div>
    ...
    <div id="moredetails-tabs">
      ...
      {if $showComments == 1}
        <div id = "readertab" >
          <div id="addReview"><span id="userreviewlink" class="add" onclick="$('#userreview{$id}').slideDown();">Add a Review</span></div>
