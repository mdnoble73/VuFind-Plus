{if count($holdings) > 0}
  ...
  <div class="eContentHolding">
    <div class="eContentHoldingHeader">
      <span class="eContentHoldingFormat">
        {if $eContentItem->externalFormat}
          {$eContentItem->externalFormat}
        {else}
          {translate text=$eContentItem->item_type}
        {/if}
      </span>
      {if $showEContentNotes}
        {$eContentItem->notes}
      {/if}
      from {$eContentItem->getSource()}
...
{if false && $availability && count($availability) > 0}
