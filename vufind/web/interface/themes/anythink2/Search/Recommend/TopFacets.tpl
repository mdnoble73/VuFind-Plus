        ...
        {if $cluster.label == 'Category'}
            {if ($categorySelected == false)}
              ...
              {foreach from=$cluster.list item=thisFacet name="narrowLoop"}
                ...
                <span class='categoryValue categoryValue{translate text=$thisFacet.value|escape}'><img class='categoryIcon' src='{$url}/interface/themes/anythink2/images/{$thisFacet.imageName|escape}.png' alt='{$thisFacet.value|escape}'/> <a href="{$thisFacet.removalUrl|escape}" class="removeFacetLink">(remove)</a></span>
                {else}
                <a href="{$thisFacet.url|escape}"><span class='categoryValue categoryValue{$thisFacet.value|escape}' ><img class='categoryIcon' src='{$url}/interface/themes/anythink2/images/{$thisFacet.imageName|escape}.png' alt='{$thisFacet.value|escape}'/>({$thisFacet.count})</span></a>
                {/if}
              {/if}
            {/foreach}
              </div>
