				...
				<div class="advancedSearchFacetDetails">
					...
					<div class="advancedSearchFacetList">
						{foreach from=$formatCategoryLimit item="value" key="display"}
							...
							<input type="radio" name="filter[]" value="{$value.filter|escape}"{if $value.selected} checked="checked"{/if} /><img src="{img filename=$value.imageName}" alt="{translate text=$display}"/><br />{translate text=$display}
							</div>
						{/foreach}
					</div>
				</div>
