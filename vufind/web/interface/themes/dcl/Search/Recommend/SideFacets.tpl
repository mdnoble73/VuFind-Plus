  ...
  {if $filterList}
    ...
    {foreach from=$filterList item=filters key=field}
      {foreach from=$filters item=filter}
        <li>{translate text=$field}: {$filter.display|escape} <a href="{$filter.removalUrl|escape}"><img src="/images/silk/delete.png" alt="Delete"/></a></li>
      {/foreach}
    {/foreach}
    </ul>
  {/if}
      ...
      {elseif $title == 'rating_facet'}
      	<dl class="narrowList navmenu narrow_begin">
	          ...
	          {if $thisFacet.isApplied}
	          	{if $curLabel == 'Unrated'}
	          		<dd>{$thisFacet.value|escape} <img src="/images/silk/tick.png" alt="Selected" /> <a href="{$thisFacet.removalUrl|escape}" class="removeFacetLink">(remove)</a></dd>
	          	{else}
	            	<dd><img src="{$path}/images/{$curLabel}.png" alt="{$curLabel|translate}"/> <img src="/images/silk/tick.png" alt="Selected" /> <a href="{$thisFacet.removalUrl|escape}" class="removeFacetLink">(remove)</a></dd>
	            {/if}
	          {else}
	          	{if $curLabel == 'Unrated'}
