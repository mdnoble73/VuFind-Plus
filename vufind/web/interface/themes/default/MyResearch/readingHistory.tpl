	          ...
	          {if ($smarty.foreach.recordLoop.iteration % 2) == 0}
					...
					<td class="titleSelectCheckedOut myAccountCell">
				    	...
				    	<div class="imageColumn"> 
						    ...
						    <a href="{$path}/{if strcasecmp($record.source, 'vufind') == 0}Record{else}EcontentRecord{/if}/{$record.recordId|escape:"url"}?searchId={$searchId}&amp;recordIndex={$recordIndex}&amp;page={$page}" id="descriptionTrigger{$record.recordId|escape:"url"}">
						    <img src="{$path}/bookcover.php?id={$record.recordId}&amp;isn={$record.isbn|@formatISBN}&amp;issn={$record.issn}&amp;size=small&amp;upc={$record.upc}&amp;category={$record.format_category|escape:"url"}" class="listResultImage" alt="{translate text='Cover Image'}"/>
						    </a>
						...
						{/if}
						    ...
						    <div class='requestThisLink' id="placeHold{$record.recordId|escape:"url"}" style="display:none">
						      <a href="{$path}/{if strcasecmp($record.source, 'vufind') == 0}Record{else}EcontentRecord{/if}/{$record.recordId|escape:"url"}/Hold"><img src="{$path}/interface/themes/default/images/place_hold.png" alt="Place Hold"/></a>
						    </div>				    
				      <div class="myAccountTitleDetails">
						  <div class="resultItemLine1">
							<a href="{$path}/{if strcasecmp($record.source, 'vufind') == 0}Record{else}EcontentRecord{/if}/{$record.recordId|escape:"url"}?searchId={$searchId}&amp;recordIndex={$recordIndex}&amp;page={$page}" class="title">{if !$record.title|removeTrailingPunctuation}{translate text='Title not available'}{else}{$record.title|removeTrailingPunctuation|truncate:180:"..."|highlight:$lookfor}{/if}</a>
							{if $record.title2}
