...
<div class="imageColumn"> 
   ...
   {/if}
    ...
    <div class='requestThisLink' id="placeHold{$listId|escape:"url"}" style="display:none">
      <a href="{$url}/EcontentRecord/{$listId|escape:"url"}/Hold"><img src="{$path}/interface/themes/default/images/place_hold.png" alt="Place Hold"/></a>
    </div>
</div>

<div class="resultDetails">
  <div class="resultItemLine1">
	<a href="{$url}/EcontentRecord/{$listId|escape:"url"}?searchId={$searchId}&amp;recordIndex={$recordIndex}&amp;page={$page}" class="title">{if !$listTitle|regex_replace:"/(\/|:)$/":""}{translate text='Title not available'}{else}{$listTitle|regex_replace:"/(\/|:)$/":""|truncate:180:"..."|highlight:$lookfor}{/if}</a>
    ...
    <script type="text/javascript">
      $(
         function() {literal} { {/literal}
             $('.rateEContent{$listId|escape}').rater({literal}{ {/literal}module: 'EcontentRecord', recordId: {$listId},  rating:0.0, postHref: '{$url}/EcontentRecord/{$listId|escape}/AJAX?method=RateTitle'{literal} } {/literal});
         {literal} } {/literal}
      );
    </script>
