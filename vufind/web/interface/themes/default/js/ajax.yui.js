...
function getLightbox(module, action, id, lookfor, message, followupModule,
		followupAction, followupId, postParams) {
	// Set Post or Get
	var ajaxMethod = (postParams == undefined || postParams == '') ? 'GET'
			: 'POST';
	var postDetails = (postParams == undefined || postParams == '') ? null
			: postParams;
	...
	var callback = {
		success : function(transaction) {
			var response = (transaction && transaction.responseText) ? transaction.responseText
	...
	var transaction = YAHOO.util.Connect.asyncRequest(ajaxMethod, url + '?'
			+ params, callback, postDetails);
...
function SaltedLogin(elems, module, action, id, lookfor, message) {
	...
	var transaction = YAHOO.util.Connect.asyncRequest('POST', url + '?' + params,
			callback, null);
}
...
function Login(elems, salt, module, action, id, lookfor, message) {
	...
	var url = path + "/AJAX/JSON";
	var params = 'method=login' + '&username=' + username + '&password='
			+ hexEncode(password);
	var callback = {
		success : function(transaction) {
			var result = eval('(' + transaction.responseText + ')');
