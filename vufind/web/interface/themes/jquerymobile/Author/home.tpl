	...
	<div data-role="content">
		{if $info}
			<div class="authorbio">
				...
				{if $info.image}
					<img src="{$info.image}" alt="{$info.altimage|escape}" style="width:150px" /><br/>
				{/if}

				{$info.description|truncate_html:4500:"...":false}

				<div class="providerLink"><a class="wikipedia" href="http://{$wiki_lang}.wikipedia.org/wiki/{$info.name|escape:"url"}" rel="external">{translate text='wiki_link'}</a></div>
			</div>
		{/if}
		{if $recordCount}
		<p>
