	...
	<div class="record" data-role="content" data-record-id="{$id}">
		...
		{if $action == 'Home' || $action == 'Holdings'}
			<div data-role="controlgroup">
				<a id="requestThisLink" href="{$path}/Record/{$id}/Hold" data-role="button" rel="external">{translate text="Place Hold"}</a>
				<a id="addToFavoritesLink" href="{$path}/Record/{$id}/Save" data-role="button" rel="external">{translate text="Add to favorites"}</a>
				<a id="addTagLink" href="{$path}/Record/{$id}/AddTag" data-role="button" rel="external">{translate text="Add Tag"}</a>
			</div>
		{/if}
		{if $subTemplate}
