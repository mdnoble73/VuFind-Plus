...
/* interface/themes/jquerymobile/js/jquery.mobile-1.2.0.min.js */
...
/* interface/themes/jquerymobile/js/common.js */
_translations=Array();$(document).bind('mobileinit',function(){$.mobile.selectmenu.prototype.options.nativeMenu=false;});$('[data-role="page"]').live('pageshow',function(){var url=location.hash;if(url.length>0){url=url.substr(1);}else{url=location.href;}
$('#langForm').attr('action',url);var match=url.match(/([&?]?ui=[^&]+)/);if(match){var replace=((match[1].indexOf('?')!=-1)?'?':'&')+'ui=standard';url=url.replace(match[1],replace);}else{url+=((url.indexOf('?')==-1)?'?':'&')+'ui=standard';}
url=url.replace('&ui-state=dialog','');$('a.standard-view').each(function(){$(this).attr('href',url);});});function pwdToText(fieldId){var elem=document.getElementById(fieldId);var input=document.createElement('input');input.id=elem.id;input.name=elem.name;input.value=elem.value;input.size=elem.size;input.onfocus=elem.onfocus;input.onblur=elem.onblur;input.className=elem.className;if(elem.type=='text'){input.type='password';}else{input.type='text';}
elem.parentNode.replaceChild(input,elem);return input;}
function moreFacets(name){$("#more"+name).hide();$(".narrowGroupHidden_"+name).show();}
function lessFacets(name){$("#more"+name).show();$(".narrowGroupHidden_"+name).hide();}
...
function ajaxLightbox(urlToLoad,parentId,left,width,top,height){$.mobile.changePage(urlToLoad,{type:"get",role:"dialog",showLoadMsg:true});}
function hideLightbox(){$('.ui-dialog').dialog('close');}
...
$.ajax({url:url,data:{username:username,password:password},success:function(response){if(response.result.success==true){loggedIn=true;$('.loginOptions').hide();$('.logoutOptions').show();$('#loginOptions').hide();$('#logoutOptions').show();$('#myAccountNameLink').html(response.result.name);if(ajaxCallback&&typeof(ajaxCallback)==="function"){ajaxCallback();}else{hideLightbox();}}else{alert("That login information was not recognized.  Please try again.");}},error:function(){alert("There was an error processing your login, please try again.");},dataType:'json',type:'post'});return false;}
function showProcessingIndicator(message){$.mobile.loading('show',{text:message,textVisible:true});}
function hideProcessingIndicator(){$.mobile.loading('hide');}
...
/* interface/themes/jquerymobile/js/jquery_cookie.js */
...
return cookieValue;}};
/* interface/themes/jquerymobile/js/cart_cookie.js */
var _CART_COOKIE='vufind_cart';var _CART_COOKIE_DELIM="\t";function getItemsFromCartCookie(){var cookie=$.cookie(_CART_COOKIE);if(cookie){var cart=cookie.split(_CART_COOKIE_DELIM);return cart?cart:Array();}
return Array();}
function addItemToCartCookie(item){var items=getItemsFromCartCookie();if(items.indexOf(item)==-1){items.push(item);$.cookie(_CART_COOKIE,items.join(_CART_COOKIE_DELIM),{path:'/'});}
return items;}
function removeItemFromCartCookie(item){var items=getItemsFromCartCookie();var index=items.indexOf(item);if(index!=-1){items.splice(index,1);}
$.cookie(_CART_COOKIE,items.join(_CART_COOKIE_DELIM),{path:'/'});return items;}
function emptyCartCookie(){var items=Array();$.cookie(_CART_COOKIE,items.join(_CART_COOKIE_DELIM),{path:'/'});return items;}
/* interface/themes/jquerymobile/js/cart.js */
$('[data-role="page"]').live('pageshow',function(){var items=getItemsFromCartCookie();updateCartSummary(items);updateCheckboxStates(items);});$('.add_to_book_bag').live('click',function(){var id=$(this).attr('data-record-id');var $icon=$('.ui-icon',this).first();if($icon.hasClass('ui-icon-plus')){$icon.removeClass('ui-icon-plus').addClass('ui-icon-check');updateCartSummary(addItemToCartCookie(id));}else if($icon.hasClass('ui-icon-check')){$icon.removeClass('ui-icon-check').addClass('ui-icon-plus');updateCartSummary(removeItemFromCartCookie(id));}
return false;});$('.remove_from_book_bag').live('click',function(){var id=$(this).attr('data-record-id');updateCartSummary(removeItemFromCartCookie(id));$li=$(this).parent('li');$ul=$li.parent('ul');if($ul.children('li').size()==1){$ul.parent().empty().append('<p>'+_translations['Your book bag is empty']+'.</p>');}else{$li.remove();}
return false;});$('.request_book_bag').live('click',function(){var url=path+"/MyResearch/HoldMultiple"
var items=getItemsFromCartCookie();$(items).each(function(i,book){if(i==0){url+="?";}else{url+="&";}
if(book.indexOf(".")==0){book=book.substring(1);}
url+="selected["+book+"]=on";});emptyCartCookie()
window.location=url;return false;});$('.empty_book_bag').live('click',function(){updateCartSummary(emptyCartCookie());$('.bookbag').parent().empty().append('<p>'+_translations['Your book bag is empty']+'.</p>');return false;});function updateCheckboxStates(items){$('.add_to_book_bag').each(function(){var id=$(this).attr('data-record-id');var $icon=$('.ui-icon',this).first();if(items.indexOf(id)!=-1){$icon.removeClass('ui-icon-plus').addClass('ui-icon-check');}else{$icon.removeClass('ui-icon-check').addClass('ui-icon-plus');}});}
function updateCartSummary(items){$summary=$('.cart_size');if($summary.size()>0){$summary.empty().append(items.length);}
$button=$('a.book_bag_btn');if($button.size()>0){$button.attr('href',$button.attr('href').replace(/Home.*/,'Home?_='+(new Date()).getTime()));}}
...
/* js/overdrive_mobile.js */
function checkoutOverDriveItem(overdriveId,formatId){if(loggedIn){if(formatId==undefined){selectOverDriveFormat(overdriveId,'checkout')}else{var url=path+"/EcontentRecord/AJAX?method=GetOverDriveLoanPeriod&overDriveId="+overdriveId+"&formatId="+formatId;ajaxLightbox(url);}}else{ajaxLogin(function(){checkoutOverDriveItem(overdriveId,formatId);});}
return false;}
...
function selectOverDriveFormat(overdriveId,nextAction){hideLightbox();var ajaxUrl=path+"/EcontentRecord/AJAX?method=SelectOverDriveFormat&overDriveId="+overdriveId+"&nextAction="+nextAction;ajaxLightbox(ajaxUrl);}
function checkoutOverDriveItemOneClick(overdriveId){if(loggedIn){showProcessingIndicator("Checking out the title for you in OverDrive.  This may take a minute.");var ajaxUrl=path+"/EcontentRecord/AJAX?method=CheckoutOverDriveItem&overDriveId="+overdriveId;$.ajax({url:ajaxUrl,cache:false,success:function(data){hideLightbox();hideProcessingIndicator();if(data.result==true){alert(data.message);$.mobile.changePage(path+"/MyResearch/OverdriveCheckedOut");}else{if(data.noCopies==true){ret=confirm(data.message)
...
function checkoutOverDriveItemStep2(overdriveId,formatId){var lendingPeriod=$("#loanPeriod option:selected").val();showProcessingIndicator("Checking out the title for you in OverDrive.  This may take a minute.");var url=path+"/EcontentRecord/AJAX?method=CheckoutOverDriveItem&overDriveId="+overdriveId+"&formatId="+formatId+"&lendingPeriod="+lendingPeriod;$.ajax({url:url,success:function(data){if(data.result){alert(data.message);$.mobile.changePage(path+"/MyResearch/OverdriveCheckedOut");}else{if(data.noCopies==true){ret=confirm(data.message)
if(ret==true){placeOverDriveHold(overdriveId,formatId);}}else{alert(data.message);hideLightbox();}}},dataType:'json',error:function(jqXHR,textStatus,errorThrown){alert("An error occurred processing your request in OverDrive. "+errorThrown+" Please try again in a few minutes.");hideLightbox();}});return false;}
...
return false;}
function doOverDriveHold(overDriveId,formatId,overdriveEmail,promptForOverdriveEmail){showProcessingIndicator("Placing a hold on the title for you in OverDrive.  This may take a minute.");var url=path+"/EcontentRecord/AJAX?method=PlaceOverDriveHold&overDriveId="+overDriveId+"&formatId="+formatId+"&overdriveEmail="+overdriveEmail+"&promptForOverdriveEmail="+promptForOverdriveEmail;$.ajax({url:url,cache:false,success:function(data){if(data.availableForCheckout){checkoutOverDriveItem(overdriveId,formatId);}else{alert(data.message);hideLightbox();hideProcessingIndicator();}},dataType:'json',async:false,error:function(){alert("An error occurred processing your request in OverDrive.  Please try again in a few minutes.");hideLightbox();}});}
...
function cancelOverDriveHold(overDriveId,formatId){if(loggedIn){showProcessingIndicator("Cancelling your hold in OverDrive.  This may take a minute.");var url=path+"/EcontentRecord/AJAX?method=CancelOverDriveHold&overDriveId="+overDriveId+"&formatId="+formatId;$.ajax({url:url,success:function(data){alert(data.message);if(data.result){window.location.href=path+"/MyResearch/OverdriveHolds";}else{hideLightbox();}},dataType:'json',error:function(){alert("An error occurred processing your request in OverDrive.  Please try again in a few minutes.");hideLightbox();}});}else{ajaxLogin(function(){cancelOverDriveHold(overDriveId,formatId);});}}
