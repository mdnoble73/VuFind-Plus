...
function checkoutOverDriveItemOneClick(overdriveId){if(loggedIn){showProcessingIndicator("Checking out the title for you in OverDrive.  This may take a minute.");var ajaxUrl=path+"/EcontentRecord/AJAX?method=CheckoutOverDriveItem&overDriveId="+overdriveId;$.ajax({url:ajaxUrl,cache:false,success:function(data){hideLightbox();if(data.result==true){alert(data.message);$.mobile.changePage(path+"/MyResearch/OverdriveCheckedOut");}else{if(data.noCopies==true){ret=confirm(data.message)
