...
function placeOverDriveHold(overDriveId,formatId){if(loggedIn){if(formatId==undefined){selectOverDriveFormat(overDriveId,'hold')}else{showProcessingIndicator("Placing a hold on the title for you in OverDrive.  This may take a minute.");setTimeout(function(){var url=path+"/EcontentRecord/AJAX?method=PlaceOverDriveHold&overDriveId="+overDriveId+"&formatId="+formatId;$.ajax({url:url,success:function(data){alert(data.message);if(data.result){window.location.href=path+"/MyResearch/OverdriveHolds";}else{hideLightbox();}},dataType:'json',error:function(){alert("An error occurred processing your request in OverDrive.  Please try again in a few minutes.");hideLightbox();}});},100);}}else{ajaxLogin(function(){placeOverDriveHold(overDriveId,formatId);});}
return false;}
