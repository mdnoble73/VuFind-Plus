...
{if $showHoldButton}
	{if $eContentRecord->isOverDrive()}
		{* Place hold link *}
		<div id="placeHoldButton"><div class='requestThisLink resultAction' id="placeEcontentHold{$summId|escape:"url"}" style="display:none">
			<a href="#" class="button" onclick="return placeOverDriveHold('{$eContentRecord->externalId}')">{translate text="Place Hold"}</a>
		</div></div>

		{* Checkout link *}
		<div id="placeHoldButton"><div class='checkoutLink resultAction' id="checkout{$summId|escape:"url"}" style="display:none">
...
{/if}
{if $showTextThis == 1}
	<li class="resultAction"><a href="{$path}/EcontentRecord/{$id|escape:"url"}/SMS" id="smsLink" onclick="ajaxLightbox('{$path}/EcontentRecord/{$id|escape}/SMS?lightbox', '#citeLink'); return false;"><span class="silk phone">&nbsp;</span>{translate text="Text this"}</a></li>
{/if}
{if $showEmailThis == 1}
	<li class="resultAction"><a href="{$path}/EcontentRecord/{$id|escape:"url"}/Email" id="mailLink" onclick="ajaxLightbox('{$path}/EcontentRecord/{$id|escape}/Email?lightbox', '#citeLink'); return false;"><span class="silk email">&nbsp;</span>{translate text="Email this"}</a></li>
{/if}
{/strip}