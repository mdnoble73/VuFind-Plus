		...
		{if count($overDriveHolds.available) > 0}
			...
			<div class='holdSectionBody'>
				<table class="myAccountTable">
					<thead>
						<tr><th>Title</th><th>Notification Sent</th><th>Expires</th><th>Manage Holds</th></tr>
					</thead>
					<tbody>
					{foreach from=$overDriveHolds.available item=record}
						<tr>
							<td rowspan="{$record.numRows}" class="myAccountCell">
                            <div class="imageColumnOverdrive"><img src="{$record.imageUrl}" alt="Cover Image" class="listResultImage"/></div>
                            <div class="myAccountOverDriveTitleDetails">
								{if $record.recordId != -1}<a href="{$path}/EcontentRecord/{$record.recordId}/Home" class="title">{/if}{$record.title}{if $record.recordId != -1}</a>{/if}
								{if $record.subTitle}<br/>{$record.subTitle}{/if}
								{if strlen($record.author) > 0}<br/>by {$record.author}{/if}
							</div>
                            </td>
							<td class="myAccountCell">{$record.notificationDate|date_format:"%a $b %e, %Y %I:%M%p"}</td>
							<td class="myAccountCell">{$record.expirationDate|date_format:"%a $b %e, %Y %I:%M%p"}</td>
