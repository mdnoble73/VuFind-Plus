{* Place hold link *}
{if $showHoldButton}
	<div id="placeHoldButton"><div class='requestThisLink resultAction button' id="placeHold{if $summShortId}{$summShortId}{else}{$summId|escape}{/if}" style="display:none">
...
{/if}
{if $showTextThis == 1}
	<li class="resultAction"><a href="{$path}/Record/{$id|escape:"url"}/SMS" onclick='ajaxLightbox("{$path}/Record/{$id|escape}/SMS?lightbox", "#smsLink"); return false;'><span class="silk phone">&nbsp;</span>{translate text="Text this"}</a></li>
{/if}
{if $showEmailThis == 1}
	<li class="resultAction"><a href="{$path}/Record/{$id|escape:"url"}/Email" onclick='ajaxLightbox("{$path}/Record/{$id|escape}/Email?lightbox", "#mailLink"); return false;'><span class="silk email">&nbsp;</span>{translate text="Email this"}</a></li>
{/if}