		...
		{foreach from=$propValue item=subObject}
			<tr id="{$propName}{$subObject->id}">
				...
				<input type="hidden" id="{$propName}Deleted_{$subObject->id}" name="{$propName}Deleted[{$subObject->id}]" value="false">
					{* link to delete *}
				<a href="#" onclick="if (confirm('Are you sure you want to delete this?')){literal}{{/literal}$('#{$propName}Deleted_{$subObject->id}').val('true');$('#{$propName}{$subObject->id}').hide().removeClass('required'){literal}}{/literal};return false;">
