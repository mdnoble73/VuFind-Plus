{if $showTagging == 1}
	<div class="sidegroup" id="tagsSidegroup">
		...
		<div class="sidebarValue">
			<a href="{$path}/Resource/AddTag?id={$id|escape:"url"}&amp;source=eContent" onclick="GetAddTagForm('{$id|escape}', 'eContent'); return false;"><span class="silk add">&nbsp;</span>{translate text="Add Tag"}</a>
		</div>
	</div>
{/if}