...
{if $statusInformation.numHolds > 0 || $statusInformation.onOrderCopies > 0}
	<div class="smallText">
		{if $statusInformation.numHolds > 0}
			{$statusInformation.numHolds} {if $statusInformation.numHolds == 1}person is{else}people are{/if} on the wait list for {$statusInformation.copies} {if $statusInformation.copies == 1}copy{else}copies{/if}
		{/if}
