{strip}
	{if $user->cat_username}
		...
		{if $profile->getNumHoldsAvailableTotal() && $profile->getNumHoldsAvailableTotal() > 0}
			<div class="text-info text-center alert alert-info">You have <span style="font-weight: bold">{$profile->getNumHoldsAvailableTotal()}</span> holds ready for pick up.</div>
		{/if}
