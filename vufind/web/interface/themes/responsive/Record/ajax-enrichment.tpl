...
{foreach from=$enrichment.novelist.relatedContent item=contentSection}
	...
	<dd>
		...
		{foreach from=$contentSection.content item=content}
			<li><a href="{$content.contentUrl}" onclick="return VuFind.Account.ajaxLightbox('{$path}/GroupedWork/AJAX?method=getNovelistData&novelistUrl={$content.contentUrl|escape:"url"}', false)">{$content.title}{if $content.author} by {$content.author}{/if}</a></li>
	  {/foreach}
		</ul>
	</dd>
{/foreach}
