...
<div id = "holdingsSummary{$holdingsSummary.shortId}" class="holdingsSummary {$holdingsSummary.class}">
	...
	{elseif $holdingsSummary.status == 'Available At'}
		<div class="holdingsSummaryStatusLine {$holdingsSummary.class}">
			...
			{else}
				...
				<a href='{$path}/{$activeRecordProfileModule}/{$holdingsSummary.recordId|escape:"url"}#holdings'>
	...
	{elseif ($holdingsSummary.status) == 'Marmot'}
		<div class="holdingsSummaryStatusLine {$holdingsSummary.class}">
			<a href='{$path}/{$activeRecordProfileModule}/{$holdingsSummary.recordId|escape:"url"}#holdings'>{translate text='Available now at'} {$holdingsSummary.numAvailableOther+$holdingsSummary.availableAt} Marmot {if $holdingsSummary.numAvailableOther == 1}Library{else}Libraries{/if}</a>
		</div>
	{elseif ($holdingsSummary.class) == 'here'}
		<div class="holdingsSummaryStatusLine {$holdingsSummary.class}">
			<a href='{$path}/{$activeRecordProfileModule}/{$holdingsSummary.recordId|escape:"url"}#holdings'>{translate text=$holdingsSummary.status} {if $holdingsSummary.location}<br/>{$holdingsSummary.location}{/if}</a>
		</div>
	{else}
		<div class="holdingsSummaryStatusLine {$holdingsSummary.class}">
			<a href='{$path}/{$activeRecordProfileModule}/{$holdingsSummary.recordId|escape:"url"}#holdings'>{translate text=$holdingsSummary.status} {if false && strlen($holdingsSummary.unavailableStatus) > 0 && $holdingsSummary.class == 'checkedOut'}({translate text=$holdingsSummary.unavailableStatus}){/if}</a>
		</div>
	{/if}
	{if $holdingsSummary.callnumber}
		<div class='callNumber'>
				<a href='{$path}/{$activeRecordProfileModule}/{$holdingsSummary.recordId|escape:"url"}#holdings'>{$holdingsSummary.callnumber}</a>
		</div>
	{/if}
	{if false && $holdingsSummary.showPlaceHold}
		<div class='requestThisLink'>
			<a href="{$path}/{$activeRecordProfileModule}/{$holdingsSummary.recordId|escape:"url"}/Hold" class="holdRequest button" style="display:inline-block;">{translate text="Request This Title"}</a><br />
		</div>
	{/if}
