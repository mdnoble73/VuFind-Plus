				...
				<div class="authorbox top-facet">
					...
					<table class="facetsTop navmenu narrow_begin">
						{foreach from=$cluster.list item=thisFacet name="narrowLoop"}
							...
							{/if}
							{if $thisFacet.isApplied}
								<td>{$thisFacet.value|escape}<img src="{$path}/images/silk/tick.png" alt="Selected" /> <a href="{$thisFacet.removalUrl|escape}" class="removeFacetLink" onclick="trackEvent('Remove Facet', '{$cluster.label}', '{$thisFacet.value|escape}');">(remove)</a></td>
							{else}
