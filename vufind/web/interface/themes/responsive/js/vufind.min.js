...
VuFind.Responsive=function(){$(function(){$(window).resize(function(){VuFind.Responsive.adjustLayout()});try{$(".collapse").on("hidden.bs.collapse",function(){VuFind.Responsive.adjustLayout()}).on("shown.bs.collapse",function(){VuFind.Responsive.adjustLayout()})}catch(err){console.log("Could not bind to resize of main content "+err)}$().ready(function(){VuFind.Responsive.adjustLayout()});$("#lookfor","#home-page-search").on("keyup",function(event){$(this).height(0);if(this.scrollHeight<32){$(this).height(18)}else{$(this).height(this.scrollHeight)}}).keyup();$("#lookfor").on("keydown",function(event){if(event.which==13||event.which==10){event.preventDefault();event.stopPropagation();$("#searchForm").submit();return false}}).on("keypress",function(event){if(event.which==13||event.which==10){event.preventDefault();event.stopPropagation();return false}})});try{var mediaQueryList=window.matchMedia("print");mediaQueryList.addListener(function(mql){VuFind.Responsive.isPrint=mql.matches})}catch(err){}window.onbeforeprint=function(){VuFind.Responsive.isPrint=true};return{resizing:false,originalSidebarHeight:-1,adjustLayout:function(){if(VuFind.Responsive.resizing){return}VuFind.Responsive.resizing=true;var resolutionX=document.documentElement.clientWidth;if(resolutionX>=768&&!VuFind.Responsive.isPrint){var mainContentElement=$("#main-content-with-sidebar");var sidebarContentElem=$("#sidebar-content");if(VuFind.Responsive.originalSidebarHeight==-1){VuFind.Responsive.originalSidebarHeight=sidebarContentElem.height()}var heightToTest=Math.min(sidebarContentElem.height(),VuFind.Responsive.originalSidebarHeight);var maxHeight=Math.max(mainContentElement.height()+15,heightToTest);if(mainContentElement.height()+15!=maxHeight){mainContentElement.height(maxHeight)}if(sidebarContentElem.height()!=maxHeight){sidebarContentElem.height(maxHeight)}}VuFind.Responsive.resizing=false}}}(VuFind.Responsive||{});
