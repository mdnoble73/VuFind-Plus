...
var Globals=function(){return{path:"",url:"",loggedIn:false,opac:false,automaticTimeoutLength:0,automaticTimeoutLengthLoggedOut:0}}(Globals||{});
var VuFind=function(){$(document).ready(function(){VuFind.initializeModalDialogs();VuFind.setupFieldSetToggles();VuFind.initCarousels();$("#modalDialog").modal({show:false});var panels=$(".panel");panels.on("show.bs.collapse",function(){$(this).addClass("active")});panels.on("hide.bs.collapse",function(){$(this).removeClass("active")})});return{changePageSize:function(){var url=window.location.href;if(url.match(/[&?]pagesize=\d+/)){url=url.replace(/pagesize=\d+/,"pagesize="+$("#pagesize").val())}else{if(url.indexOf("?",0)>0){url=url+"&pagesize="+$("#pagesize").val()}else{url=url+"?pagesize="+$("#pagesize").val()}}window.location.href=url},closeLightbox:function(callback){var modalDialog=$("#modalDialog");if(modalDialog.is(":visible")){modalDialog.modal("hide");if(callback!=undefined){var closeLightboxListener=modalDialog.on("hidden.bs.modal",function(e){modalDialog.off("hidden.bs.modal");callback()})}}},initCarousels:function(){var jcarousel=$(".jcarousel");jcarousel.on("jcarousel:reload jcarousel:create",function(){var element=$(this);var width=element.innerWidth();var itemWidth=width;if(width>=600){itemWidth=width/4}else if(width>=400){itemWidth=width/3}else if(width>=300){itemWidth=width/2}element.jcarousel("items").css("width",Math.floor(itemWidth)+"px")}).jcarousel({wrap:"circular"});$(".jcarousel-control-prev").jcarouselControl({target:"-=1"});$(".jcarousel-control-next").jcarouselControl({target:"+=1"});$(".jcarousel-pagination").on("jcarouselpagination:active","a",function(){$(this).addClass("active")}).on("jcarouselpagination:inactive","a",function(){$(this).removeClass("active")}).on("click",function(e){e.preventDefault()}).jcarouselPagination({perPage:1,item:function(page){return'<a href="#'+page+'">'+page+"</a>"}});if(typeof VuFind.Browse.initializeBrowseCategory=="function"){VuFind.Browse.initializeBrowseCategory()}},initializeModalDialogs:function(){$(".modalDialogTrigger").each(function(){$(this).click(function(){var trigger=$(this);var dialogTitle=trigger.attr("title")?trigger.attr("title"):trigger.data("title");var dialogDestination=trigger.attr("href");$("#myModalLabel").text(dialogTitle);$(".modal-body").load(dialogDestination);$(".extraModalButton").hide();$("#modalDialog").modal("show");return false})})},getQuerystringParameters:function(){var vars=[],q=location.search.substr(1);if(q!=undefined){q=q.split("&");for(var i=0;i<q.length;i++){var hash=q[i].split("=");vars[hash[0]]=hash[1]}}return vars},getSelectedTitles:function(){var selectedTitles=$("input.titleSelect:checked ").map(function(){return $(this).attr("name")+"="+$(this).val()}).get().join("&");if(selectedTitles.length==0){var ret=confirm("You have not selected any items, process all items?");if(ret==true){var titleSelect=$("input.titleSelect");titleSelect.attr("checked","checked");selectedTitles=titleSelect.map(function(){return $(this).attr("name")+"="+$(this).val()}).get().join("&")}}return selectedTitles},pwdToText:function(fieldId){var elem=document.getElementById(fieldId);var input=document.createElement("input");input.id=elem.id;input.name=elem.name;input.value=elem.value;input.size=elem.size;input.onfocus=elem.onfocus;input.onblur=elem.onblur;input.className=elem.className;if(elem.type=="text"){input.type="password"}else{input.type="text"}elem.parentNode.replaceChild(input,elem);return input},setupFieldSetToggles:function(){$("legend.collapsible").each(function(){$(this).siblings().hide().addClass("collapsed").click(function(){$(this).toggleClass("expanded collapsed").siblings().slideToggle();return false})});$("fieldset.fieldset-collapsible").each(function(){var collapsible=$(this);var legend=collapsible.find("legend:first");legend.addClass("fieldset-collapsible-label").bind("click",{collapsible:collapsible},function(event){var collapsible=event.data.collapsible;if(collapsible.hasClass("fieldset-collapsed")){collapsible.removeClass("fieldset-collapsed")}else{collapsible.addClass("fieldset-collapsed")}});collapsible.addClass("fieldset-collapsed")})},showMessage:function(title,body,autoClose,refreshAfterClose){if(autoClose==undefined){autoClose=false}if(refreshAfterClose==undefined){refreshAfterClose=false}$("#myModalLabel").html(title);$(".modal-body").html(body);$(".modal-buttons").html("");var modalDialog=$("#modalDialog");modalDialog.modal("show");if(autoClose){setTimeout(function(){if(refreshAfterClose)location.reload(true);else VuFind.closeLightbox()},autoClose>1?autoClose:3e3)}else if(refreshAfterClose){modalDialog.on("hide.bs.modal",function(){location.reload(true)})}},showMessageWithButtons:function(title,body,buttons){$("#myModalLabel").html(title);$(".modal-body").html(body);$(".modal-buttons").html(buttons);var modalDialog=$("#modalDialog");modalDialog.modal("show")},loadingMessage:function(){VuFind.showMessage("Loading","Loading, please wait.")},ajaxFail:function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")},toggleHiddenElementWithButton:function(button){var hiddenElementName=$(button).data("hidden_element");var hiddenElement=$(hiddenElementName);hiddenElement.val($(button).hasClass("active")?"1":"0");return false},showElementInPopup:function(title,elementId){var modalDialog=$("#modalDialog");if(modalDialog.is(":visible")){VuFind.closeLightbox(function(){VuFind.showElementInPopup(title,elementId)})}else{$(".modal-title").html(title);var elementText=$(elementId).html();$(".modal-body").html(elementText);var modalDialog=$("#modalDialog");modalDialog.modal("show");return false}},showLocationHoursAndMap:function(){var selectedId=$("#selectLibrary").find(":selected").val();$(".locationInfo").hide();$("#locationAddress"+selectedId).show();return false},toggleCheckboxes:function(checkboxSelector,toggleSelector){var toggle=$(toggleSelector);var value=toggle.prop("checked");$(checkboxSelector).prop("checked",value)},submitOnEnter:function(event,formToSubmit){if(event.keyCode==13){$(formToSubmit).submit()}},hasLocalStorage:function(){if(typeof arguments.callee.haslocalStorage=="undefined"){if("localStorage"in window){try{window.localStorage.setItem("_tmptest","temp");arguments.callee.haslocalStorage=window.localStorage.getItem("_tmptest")=="temp";window.localStorage.removeItem("_tmptest")}catch(error){arguments.callee.haslocalStorage=false}}else arguments.callee.haslocalStorage=false}return arguments.callee.haslocalStorage}}}(VuFind||{});jQuery.validator.addMethod("multiemail",function(value,element){if(this.optional(element)){return true}var emails=value.split(/[,;]/),valid=true;for(var i=0,limit=emails.length;i<limit;i++){value=emails[i];valid=valid&&jQuery.validator.methods.email.call(this,value,element)}return valid},"Invalid email format: please use a comma to separate multiple email addresses.");jQuery.validator.addMethod("datePika",function(value,element){var check=false;var re=/^\d{1,2}(-)\d{1,2}(-)\d{4}$/;if(re.test(value)){var adata=value.split("-");var mm=parseInt(adata[0],10);var dd=parseInt(adata[1],10);var aaaa=parseInt(adata[2],10);var xdata=new Date(aaaa,mm-1,dd);if(xdata.getFullYear()==aaaa&&xdata.getMonth()==mm-1&&xdata.getDate()==dd)check=true;else check=false}else check=false;return this.optional(element)||check},"Please enter a correct date");
VuFind.Account=function(){return{ajaxCallback:null,closeModalOnAjaxSuccess:false,addAccountLink:function(){var url=Globals.path+"/MyAccount/AJAX?method=getAddAccountLinkForm";VuFind.Account.ajaxLightbox(url,true)},addList:function(){var form=$("#addListForm"),isPublic=form.find("#public").prop("checked"),recordId=form.find("input[name=recordId]").val(),source=form.find("input[name=source]").val(),title=form.find("input[name=title]").val(),desc=$("#listDesc").val(),url=Globals.path+"/MyAccount/AJAX",params={method:"AddList",title:title,"public":isPublic,desc:desc,recordId:recordId};$.getJSON(url,params,function(data){if(data.result){VuFind.showMessage("Added Successfully",data.message,true)}else{VuFind.showMessage("Error",data.message)}}).fail(function(){VuFind.showMessage("Error creating list","There was an unexpected error creating your list")});return false},ajaxLogin:function(trigger,ajaxCallback,closeModalOnAjaxSuccess){if(Globals.loggedIn){if(ajaxCallback!=undefined&&typeof ajaxCallback==="function"){ajaxCallback()}else if(VuFind.Account.ajaxCallback!=null&&typeof VuFind.Account.ajaxCallback==="function"){VuFind.Account.ajaxCallback();VuFind.Account.ajaxCallback=null}}else{var multistep=false,loginLink=false;if(ajaxCallback!=undefined&&typeof ajaxCallback==="function"){multistep=true}VuFind.Account.ajaxCallback=ajaxCallback;VuFind.Account.closeModalOnAjaxSuccess=closeModalOnAjaxSuccess;if(trigger!=undefined&&trigger!=null){var dialogTitle=trigger.attr("title")?trigger.attr("title"):trigger.data("title");loginLink=trigger.data("login")}var dialogDestination=Globals.path+"/MyAccount/AJAX?method=LoginForm";if(multistep&&!loginLink){dialogDestination+="&multistep=true"}var modalDialog=$("#modalDialog");$(".modal-body").html("Loading...");$(".modal-content").load(dialogDestination);$(".modal-title").text(dialogTitle);modalDialog.modal("show")}return false},followLinkIfLoggedIn:function(trigger,linkDestination){if(trigger==undefined){alert("You must provide the trigger to follow a link after logging in.")}var jqTrigger=$(trigger);if(linkDestination==undefined){linkDestination=jqTrigger.attr("href")}this.ajaxLogin(jqTrigger,function(){document.location=linkDestination},true);return false},preProcessLogin:function(){var username=$("#username").val(),password=$("#password").val(),loginErrorElem=$("#loginError");if(!username||!password){loginErrorElem.text($("#missingLoginPrompt").text()).show();return false}if(VuFind.hasLocalStorage()){var rememberMe=$("#rememberMe").prop("checked"),showPwd=$("#showPwd").prop("checked");if(rememberMe){window.localStorage.setItem("lastUserName",username);window.localStorage.setItem("lastPwd",password);window.localStorage.setItem("showPwd",showPwd);window.localStorage.setItem("rememberMe",rememberMe)}else{window.localStorage.removeItem("lastUserName");window.localStorage.removeItem("lastPwd");window.localStorage.removeItem("showPwd");window.localStorage.removeItem("rememberMe")}}return true},processAddLinkedUser:function(){if(this.preProcessLogin()){var username=$("#username").val(),password=$("#password").val(),loginErrorElem=$("#loginError"),url=Globals.path+"/MyAccount/AJAX?method=addAccountLink";loginErrorElem.hide();$.ajax({url:url,data:{username:username,password:password},success:function(response){if(response.result==true){VuFind.showMessage("Account to Manage","Successfully linked the account.",true,true)}else{loginErrorElem.text(response.result.message);loginErrorElem.show()}},error:function(){loginErrorElem.text("There was an error processing the account, please try again.").show()},dataType:"json",type:"post"})}return false},processAjaxLogin:function(ajaxCallback){if(this.preProcessLogin()){var username=$("#username").val(),password=$("#password").val(),rememberMe=$("#rememberMe").prop("checked"),loginErrorElem=$("#loginError"),url=Globals.path+"/AJAX/JSON?method=loginUser";loginErrorElem.hide();$.ajax({url:url,data:{username:username,password:password,rememberMe:rememberMe},success:function(response){if(response.result.success==true){$(".loginOptions, #loginOptions").hide();$(".logoutOptions, #logoutOptions").show();var name=response.result.name.trim();$("#header-container #myAccountNameLink").html(name);name="Logged In As "+name.slice(0,name.lastIndexOf(" ")+2)+".";$("#side-bar #myAccountNameLink").html(name);if(VuFind.Account.closeModalOnAjaxSuccess){VuFind.closeLightbox()}Globals.loggedIn=true;if(ajaxCallback!=undefined&&typeof ajaxCallback==="function"){ajaxCallback()}else if(VuFind.Account.ajaxCallback!=undefined&&typeof VuFind.Account.ajaxCallback==="function"){VuFind.Account.ajaxCallback();VuFind.Account.ajaxCallback=null}}else{loginErrorElem.text(response.result.message);loginErrorElem.show()}},error:function(){loginErrorElem.text("There was an error processing your login, please try again.").show()},dataType:"json",type:"post"})}return false},removeLinkedUser:function(idToRemove){if(confirm("Are you sure you want to stop managing this account?")){var url=Globals.path+"/MyAccount/AJAX?method=removeAccountLink&idToRemove="+idToRemove;$.getJSON(url,function(data){if(data.result==true){VuFind.showMessage("Linked Account Removed",data.message,true,true)}else{VuFind.showMessage("Unable to Remove Account Link",data.message)}})}return false},removeTag:function(tag){if(confirm('Are you sure you want to remove the tag "'+tag+'" from all titles?')){var url=Globals.path+"/MyAccount/AJAX",params={method:"removeTag",tag:tag};$.getJSON(url,params,function(data){if(data.result==true){VuFind.showMessage("Tag Deleted",data.message,true,true)}else{VuFind.showMessage("Tag Not Deleted",data.message)}})}return false},renewTitle:function(patronId,recordId,renewIndicator){if(Globals.loggedIn){VuFind.loadingMessage();$.getJSON(Globals.path+"/MyAccount/AJAX?method=renewItem&patronId="+patronId+"&recordId="+recordId+"&renewIndicator="+renewIndicator,function(data){VuFind.showMessage(data.title,data.modalBody,data.success,data.success)}).fail(VuFind.ajaxFail)}else{this.ajaxLogin(null,function(){this.renewTitle(renewIndicator)},false)}return false},renewAll:function(){if(!Globals.loggedIn){this.ajaxLogin(null,function(){this.renewAll()},false)}else if(confirm("Renew All Items?")){VuFind.showMessage("Loading","Loading, please wait");$.getJSON(Globals.path+"/MyAccount/AJAX?method=renewAll",function(data){VuFind.showMessage(data.title,data.modalBody,data.success);if(data.success||data.renewed>0){$("#modalDialog").on("hidden.bs.modal",function(e){location.reload(true)})}}).fail(function(){VuFind.ajaxFail()})}return false},renewSelectedTitles:function(){if(!Globals.loggedIn){this.ajaxLogin(null,function(){this.renewSelectedTitles()},false)}else{var selectedTitles=VuFind.getSelectedTitles();if(selectedTitles){if(confirm("Renew selected Items?")){VuFind.showMessage("Loading","Loading, please wait");$.getJSON(Globals.path+"/MyAccount/AJAX?method=renewSelectedItems&"+selectedTitles,function(data){var reload=data.success||data.renewed>0;VuFind.showMessage(data.title,data.modalBody,data.success,reload)}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")})}}}return false},resetPin:function(){var barcode=$("#card_number").val();if(barcode.length==0){alert("Please enter your library card number")}else{var url=path+"/MyAccount/AJAX?method=requestPinReset&barcode="+barcode;$.getJSON(url,function(data){if(data.error==false){alert(data.message);if(data.result==true){hideLightbox()}}else{alert("There was an error requesting your pin reset information.  Please contact the library for additional information.")}})}return false},ajaxLightbox:function(urlToDisplay,requireLogin){if(requireLogin==undefined){requireLogin=false}if(requireLogin&&!Globals.loggedIn){VuFind.Account.ajaxLogin(null,function(){VuFind.Account.ajaxLightbox(urlToDisplay,requireLogin)},false)}else{var modalDialog=$("#modalDialog");$("#myModalLabel").html("Loading, please wait");$(".modal-body").html("...");$.getJSON(urlToDisplay,function(data){if(data.success){data=data.result}$("#myModalLabel").html(data.title);$(".modal-body").html(data.modalBody);$(".modal-buttons").html(data.modalButtons)});modalDialog.modal("show")}return false},cancelHold:function(patronId,recordId,holdIdToCancel){if(confirm("Are you sure you want to cancel this hold?")){if(Globals.loggedIn){VuFind.showMessage("Loading","Loading, please wait");$.getJSON(Globals.path+"/MyAccount/AJAX?method=cancelHold&patronId="+patronId+"&recordId="+recordId+"&cancelId="+holdIdToCancel,function(data){VuFind.showMessage(data.title,data.modalBody,data.success);if(data.success){var escapedHoldId=holdIdToCancel.replace("~","\\~");$("div.result").has("#selected"+escapedHoldId).remove()}}).fail(function(){VuFind.ajaxFail()})}else{this.ajaxLogin(null,function(){VuFind.Account.cancelHold(userId,holdIdToCancel)},false)}}return false},cancelSelectedHolds:function(){if(Globals.loggedIn){var selectedTitles=this.getSelectedTitles().replace(/waiting|available/g,""),numHolds=$("input.titleSelect:checked").length;if(numHolds>0&&confirm("Cancel "+numHolds+" selected hold"+(numHolds>1?"s":"")+"?")){VuFind.loadingMessage();$.getJSON(Globals.path+"/MyAccount/AJAX?method=cancelHolds&"+selectedTitles,function(data){VuFind.showMessage(data.title,data.modalBody,data.success);if(data.success){$("input.titleSelect:checked").closest("div.result").remove()}else if(data.failed){var searchArray=data.failed.map(function(ele){return ele.toString()});$("input.titleSelect:checked").each(function(){var id=$(this).attr("id").replace(/selected/g,"");if($.inArray(id,searchArray)==-1)$(this).closest("div.result").remove()})}}).fail(function(){VuFind.ajaxFail()})}}else{this.ajaxLogin(null,function(){VuFind.Account.cancelSelectedHolds()},false)}return false},cancelBooking:function(cancelId){if(confirm("Are you sure you want to cancel this booking?")){if(Globals.loggedIn){VuFind.loadingMessage();$.getJSON(Globals.path+"/MyAccount/AJAX",{method:"cancelBooking",cancelId:cancelId},function(data){VuFind.showMessage(data.title,data.modalBody,data.success);if(data.success){var escapedId=cancelId.replace(/:/g,"\\:");$("div.result").has("#selected"+escapedId).remove()}}).fail(VuFind.ajaxFail)}else{this.ajaxLogin(null,function(){VuFind.Account.cancelBooking(cancelId)},false)}}return false},cancelSelectedBookings:function(){if(Globals.loggedIn){var selectedTitles=this.getSelectedTitles(),numBookings=$("input.titleSelect:checked").length;if(numBookings>0&&confirm("Cancel "+numBookings+" selected booking"+(numBookings>1?"s":"")+"?")){VuFind.loadingMessage();$.getJSON(Globals.path+"/MyAccount/AJAX?method=cancelBooking&"+selectedTitles,function(data){VuFind.showMessage(data.title,data.modalBody,data.success);if(data.success){$("input.titleSelect:checked").closest("div.result").remove()}else if(data.failed){var searchArray=data.failed.map(function(ele){return ele.toString()});$("input.titleSelect:checked").each(function(){var id=$(this).attr("id").replace(/selected/g,"");if($.inArray(id,searchArray)==-1)$(this).closest("div.result").remove()})}}).fail(VuFind.ajaxFail)}}else{this.ajaxLogin(null,VuFind.Account.cancelSelectedBookings,false)}return false},cancelAllBookings:function(){if(Globals.loggedIn){if(confirm("Cancel all of your bookings?")){VuFind.loadingMessage();$.getJSON(Globals.path+"/MyAccount/AJAX?method=cancelBooking&cancelAll=1",function(data){VuFind.showMessage(data.title,data.modalBody,data.success);if(data.success){$("input.titleSelect").closest("div.result").remove()}else if(data.failed){var searchArray=data.failed.map(function(ele){return ele.toString()});$("input.titleSelect").each(function(){var id=$(this).attr("id").replace(/selected/g,"");if($.inArray(id,searchArray)==-1)$(this).closest("div.result").remove()})}}).fail(VuFind.ajaxFail)}}else{this.ajaxLogin(null,VuFind.Account.cancelAllBookings,false)}return false},changeAccountSort:function(newSort){var currentLocation=window.location.href;if(currentLocation.match(/(accountSort=[^&]*)/)){currentLocation=currentLocation.replace(/accountSort=[^&]*/,"accountSort="+newSort)}else{if(currentLocation.match(/\?/)){currentLocation+="&accountSort="+newSort}else{currentLocation+="?accountSort="+newSort}}window.location.href=currentLocation},changeHoldPickupLocation:function(patronId,recordId,holdId){if(Globals.loggedIn){var modalDialog=$("#modalDialog");$("#myModalLabel").html("Loading");$(".modal-body").html("");$.getJSON(Globals.path+"/MyAccount/AJAX?method=getChangeHoldLocationForm&patronId="+patronId+"&recordId="+recordId+"&holdId="+holdId,function(data){$("#myModalLabel").html(data.title);$(".modal-body").html(data.modalBody);$(".modal-buttons").html(data.modalButtons)});modalDialog.modal("show")}else{VuFind.Account.ajaxLogin(null,function(){return VuFind.Account.changeHoldPickupLocation(patronId,recordId,holdId)},false)}return false},deleteSearch:function(searchId){if(!Globals.loggedIn){VuFind.Account.ajaxLogin(null,function(){VuFind.Searches.saveSearch(searchId)},false)}else{var url=Globals.path+"/MyAccount/AJAX";var params="method=deleteSearch&searchId="+encodeURIComponent(searchId);$.getJSON(url+"?"+params,function(data){if(data.result){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Error",data.message)}})}return false},doChangeHoldLocation:function(){var patronId=$("#patronId").val();var recordId=$("#recordId").val();var holdId=$("#holdId").val();var newLocation=$("#newPickupLocation").val();var url=Globals.path+"/MyAccount/AJAX?method=changeHoldLocation&patronId="+patronId+"&recordId="+recordId+"&holdId="+encodeURIComponent(holdId)+"&newLocation="+encodeURIComponent(newLocation);$.getJSON(url,function(data){if(data.success){VuFind.showMessage("Success",data.message,true,true)}else{VuFind.showMessage("Error",data.message)}})},freezeHold:function(patronId,recordId,holdId,promptForReactivationDate,caller){if(promptForReactivationDate){var modalDialog=$("#modalDialog");$.getJSON(Globals.path+"/MyAccount/AJAX?method=getReactivationDateForm&patronId="+patronId+"&recordId="+recordId+"&holdId="+holdId,function(data){$("#myModalLabel").html(data.title);$(".modal-body").html(data.modalBody);$(".modal-buttons").html(data.modalButtons)});modalDialog.load();modalDialog.modal("show")}else{var popUpBoxTitle=$(caller).text()||"Freezing Hold";VuFind.showMessage(popUpBoxTitle,"Updating your hold.  This may take a minute.");var url=Globals.path+"/MyAccount/AJAX?method=freezeHold&patronId="+patronId+"&recordId="+recordId+"&holdId="+holdId;$.getJSON(url,function(data){if(data.success){VuFind.showMessage("Success",data.message,true,true)}else{VuFind.showMessage("Error",data.message)}})}},doFreezeHoldWithReactivationDate:function(caller){var popUpBoxTitle=$(caller).text()||"Freezing Hold";var patronId=$("#patronId").val();var recordId=$("#recordId").val();var holdId=$("#holdId").val();var reactivationDate=$("#reactivationDate").val();var url=Globals.path+"/MyAccount/AJAX?method=freezeHold&patronId="+patronId+"&recordId="+recordId+"&holdId="+holdId+"&reactivationDate="+reactivationDate;VuFind.showMessage(popUpBoxTitle,"Updating your hold.  This may take a minute.");$.getJSON(url,function(data){if(data.success){VuFind.showMessage("Success",data.message,true,true)}else{VuFind.showMessage("Error",data.message)}})},freezeSelectedHolds:function(){var selectedTitles=this.getSelectedTitles();if(selectedTitles.length==0){return false}var suspendDate="",suspendDateTop=$("#suspendDateTop"),url="",queryParams="";if(suspendDateTop.length){if(suspendDateTop.val().length>0){suspendDate=suspendDateTop.val()}else{suspendDate=$("#suspendDateBottom").val()}if(suspendDate.length==0){alert("Please select the date when the hold should be reactivated.");return false}}url=Globals.path+"/MyAccount/Holds?multiAction=freezeSelected&patronId="+patronId+"&recordId="+recordId+"&"+selectedTitles+"&suspendDate="+suspendDate;queryParams=VuFind.getQuerystringParameters();if($.inArray("section",queryParams)){url+="&section="+queryParams["section"]}window.location=url;return false},getSelectedTitles:function(promptForSelectAll){if(promptForSelectAll==undefined){promptForSelectAll=true}var selectedTitles=$("input.titleSelect:checked ");if(selectedTitles.length==0&&promptForSelectAll&&confirm("You have not selected any items, process all items?")){selectedTitles=$("input.titleSelect").attr("checked","checked")}var queryString=selectedTitles.map(function(){return $(this).attr("name")+"="+$(this).val()}).get().join("&");return queryString},saveSearch:function(searchId){if(!Globals.loggedIn){VuFind.Account.ajaxLogin(null,function(){VuFind.Searches.saveSearch(searchId)},false)}else{var url=Globals.path+"/MyAccount/AJAX";var params={method:"saveSearch",searchId:searchId};$.getJSON(url,params,function(data){if(data.success){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Error",data.message)}})}return false},showCreateListForm:function(id){if(Globals.loggedIn){var modalDialog=$("#modalDialog");var url=Globals.path+"/MyAccount/AJAX",params={method:"getCreateListForm"};if(id!=undefined){params.recordId=id}$.getJSON(url,params,function(data){$("#myModalLabel").html(data.title);$(".modal-body").html(data.modalBody);$(".modal-buttons").html(data.modalButtons)});modalDialog.modal("show")}else{VuFind.Account.ajaxLogin($trigger,function(){return VuFind.GroupedWork.showEmailForm(trigger,id)},false)}return false},thawHold:function(patronId,recordId,holdId,caller){var popUpBoxTitle=$(caller).text()||"Thawing Hold";VuFind.showMessage(popUpBoxTitle,"Updating your hold.  This may take a minute.");var url=Globals.path+"/MyAccount/AJAX?method=thawHold&patronId="+patronId+"&recordId="+recordId+"&holdId="+holdId;$.getJSON(url,function(data){if(data.success){VuFind.showMessage("Success",data.message,true,true)}else{VuFind.showMessage("Error",data.message)}})}}}(VuFind.Account||{});
VuFind.Admin=function(){return{showReindexNotes:function(id){VuFind.Account.ajaxLightbox("/Admin/AJAX?method=getReindexNotes&id="+id,true);return false},toggleReindexProcessInfo:function(id){$("#reindexEntry"+id).toggleClass("expanded collapsed");$("#processInfo"+id).toggle()},showReindexProcessNotes:function(id){VuFind.Account.ajaxLightbox("/Admin/AJAX?method=getReindexProcessNotes&id="+id,true);return false},showCronNotes:function(id){VuFind.Account.ajaxLightbox("/Admin/AJAX?method=getCronNotes&id="+id,true);return false},showCronProcessNotes:function(id){VuFind.Account.ajaxLightbox("/Admin/AJAX?method=getCronProcessNotes&id="+id,true);return false},toggleCronProcessInfo:function(id){$("#cronEntry"+id).toggleClass("expanded collapsed");$("#processInfo"+id).toggle()},showOverDriveExtractNotes:function(id){VuFind.Account.ajaxLightbox("/Admin/AJAX?method=getOverDriveExtractNotes&id="+id,true);return false}}}(VuFind.Admin||{});
...
VuFind.EContent=function(){return{submitHelpForm:function(){var url=Globals.path+"/Help/eContentSupport";$.ajax({type:"POST",url:url,data:$("#eContentSupport").serialize(),success:function(data){var jsonData=JSON.parse(data);VuFind.showMessage(jsonData.title,jsonData.message)},failure:function(data){alert("Could not submit the form")}});return false}}}(VuFind.EContent);VuFind.ExternalEContentRecord=function(){return{loadHoldingsInfo:function(id,type,callback){var url=Globals.path+"/ExternalEContent/"+encodeURIComponent(id)+"/AJAX";var params="method=GetHoldingsInfo";var fullUrl=url+"?"+params;$.ajax({url:fullUrl,success:function(data){var holdingsData=$(data).find("Formats").text();if(holdingsData){var formatsPlaceholder=$("#formatsPlaceholder");if(holdingsData.length>0){formatsPlaceholder.html(holdingsData);formatsPlaceholder.trigger("create")}else{formatsPlaceholder.html("No Formats Information found, please try again later.")}}var copiesData=$(data).find("Copies").text();var copiesPlaceholder=$("#copiesPlaceholder");if(copiesData){if(copiesData.length>0){copiesPlaceholder.html(copiesData);copiesPlaceholder.trigger("create")}else{$("#copiestabLink").hide();$("#copiesPlaceholder").html("No Copies Information found, please try again later.");$("#formatstabLink a").text("Copies")}}else{$("#copiestabLink").hide();copiesPlaceholder.html("No Copies Information found, please try again later.");$("#formatstabLink a").text("Copies")}var holdingsSummary=$(data).find("HoldingsSummary").text();if(holdingsSummary){if(holdingsSummary.length>0){$("#holdingsSummaryPlaceholder").html(holdingsSummary);$("#holdingsSummaryPlaceholder").trigger("create")}}var showPlaceHold=$(data).find("ShowPlaceHold").text();if(showPlaceHold){if(showPlaceHold.length>0&&showPlaceHold==1){$(".requestThisLink").show()}}var showCheckout=$(data).find("ShowCheckout").text();if(showCheckout){if(showCheckout.length>0&&showCheckout==1){$(".checkoutLink").show()}}var showAccessOnline=$(data).find("ShowAccessOnline").text();if(showAccessOnline){if(showAccessOnline.length>0&&showAccessOnline==1){if($(data).find("AccessOnlineUrl").length>0){var url=$(data).find("AccessOnlineUrl").text();var text=$(data).find("AccessOnlineText").text();$("#accessOnline"+id+" a").attr("href",url);$("#accessOnline"+id+" a").text($("<div/>").html(text).text())}$(".accessOnlineLink").show()}}var showAddToWishList=$(data).find("ShowAddToWishlist").text();if(showAddToWishList){if(showAddToWishList.length>0&&showAddToWishList==1){$(".addToWishListLink").show()}}var status=$(data).find("status").text();$("#statusValue").html(status);$("#statusValue").addClass($(data).find("class").text());if(typeof callback==="function"){callback()}}})}}}(VuFind.ExternalEContentRecord);VuFind.LocalEContent=function(){return{cancelHold:function(recordId,itemId){if(confirm("Are you sure you want to cancel this hold?")){if(Globals.loggedIn){var returnUrl=Globals.path+"/RestrictedEContent/"+recordId+"/AJAX?method=cancelHold&itemId="+itemId;$.getJSON(returnUrl,function(data){if(data.result){VuFind.showMessage("Success",data.message,true,true)}else{VuFind.showMessage("Error",data.message)}})}else{VuFind.Account.ajaxLogin(null,function(){VuFind.LocalEContent.cancelHold(recordId,itemId)})}}},checkoutPublicEContent:function(recordId,itemId){if(Globals.loggedIn){var checkoutUrl=Globals.path+"/PublicEContent/"+recordId+"/AJAX?method=checkout&itemId="+itemId;$.getJSON(checkoutUrl,function(data){if(data.result){VuFind.showMessage("Success",data.message,true,true)}else{VuFind.showMessage("Error",data.message)}})}else{VuFind.Account.ajaxLogin(null,function(){VuFind.LocalEContent.checkoutPublicEContent(recordId,itemId)},false)}},checkoutRestrictedEContent:function(recordId,itemId){if(Globals.loggedIn){var checkoutUrl=Globals.path+"/RestrictedEContent/"+recordId+"/AJAX?method=checkout&itemId="+itemId;$.getJSON(checkoutUrl,function(data){if(data.result){VuFind.showMessage("Success",data.message,true,true)}else{VuFind.showMessage("Error",data.message)}})}else{VuFind.Account.ajaxLogin(null,function(){VuFind.LocalEContent.checkoutRestrictedEContent(recordId,itemId)})}},placeHoldOnRestrictedEContent:function(recordId,itemId){if(Globals.loggedIn){var returnUrl=Globals.path+"/RestrictedEContent/"+recordId+"/AJAX?method=placeHold&itemId="+itemId;$.getJSON(returnUrl,function(data){if(data.result){VuFind.showMessage("Success",data.message,true,true)}else{VuFind.showMessage("Error",data.message)}})}else{VuFind.Account.ajaxLogin(null,function(){VuFind.LocalEContent.placeHoldOnRestrictedEContent(recordId,itemId)})}},returnPublicEContent:function(recordId,itemId){if(Globals.loggedIn){var returnUrl=Globals.path+"/PublicEContent/"+recordId+"/AJAX?method=returnTitle&itemId="+itemId;$.getJSON(returnUrl,function(data){if(data.result){VuFind.showMessage("Success",data.message,true,true)}else{VuFind.showMessage("Error",data.message)}})}else{VuFind.Account.ajaxLogin(null,function(){VuFind.LocalEContent.returnPublicEContent(recordId,itemId)})}},returnRestrictedEContent:function(recordId,itemId){if(Globals.loggedIn){var returnUrl=Globals.path+"/RestrictedEContent/"+recordId+"/AJAX?method=returnTitle&itemId="+itemId;$.getJSON(returnUrl,function(data){if(data.result){VuFind.showMessage("Success",data.message,true,true)}else{VuFind.showMessage("Error",data.message)}})}else{VuFind.Account.ajaxLogin(null,function(){VuFind.LocalEContent.returnRestrictedEContent(recordId,itemId)})}}}}(VuFind.LocalEContent);
VuFind.GroupedWork=function(){return{hasTableOfContentsInRecord:false,clearUserRating:function(groupedWorkId){var url=Globals.path+"/GroupedWork/"+groupedWorkId+"/AJAX?method=clearUserRating";$.getJSON(url,function(data){if(data.result==true){$(".rate"+groupedWorkId).find(".ui-rater-starsOn").width(0);$("#myRating"+groupedWorkId).hide();VuFind.showMessage("Success",data.message,true)}else{VuFind.showMessage("Sorry",data.message)}});return false},clearNotInterested:function(notInterestedId){var url=Globals.path+"/GroupedWork/"+notInterestedId+"/AJAX?method=clearNotInterested";$.getJSON(url,function(data){if(data.result==false){VuFind.showMessage("Sorry","There was an error updating the title.")}else{$("#notInterested"+notInterestedId).hide()}})},deleteReview:function(id,reviewId){if(confirm("Are you sure you want to delete this review?")){var url=Globals.path+"/GroupedWork/"+id+"/AJAX?method=deleteUserReview";$.getJSON(url,function(data){if(data.result==true){$("#review_"+reviewId).hide();VuFind.showMessage("Success",data.message,true)}else{VuFind.showMessage("Sorry",data.message)}})}return false},forceReindex:function(id){var url=Globals.path+"/GroupedWork/"+id+"/AJAX?method=forceReindex";$.getJSON(url,function(data){VuFind.showMessage("Success",data.message,true,true);setTimeout("VuFind.closeLightbox();",3e3)});return false},getGoDeeperData:function(id,dataType){var placeholder;if(dataType=="excerpt"){placeholder=$("#excerptPlaceholder")}else if(dataType=="avSummary"){placeholder=$("#avSummaryPlaceholder")}else if(dataType=="tableOfContents"){placeholder=$("#tableOfContentsPlaceholder")}if(placeholder.hasClass("loaded"))return;placeholder.show();var url=Globals.path+"/GroupedWork/"+encodeURIComponent(id)+"/AJAX";var params="method=GetGoDeeperData&dataType="+encodeURIComponent(dataType);var fullUrl=url+"?"+params;$.getJSON(fullUrl,function(data){placeholder.html(data.formattedData);placeholder.addClass("loaded")})},getGoodReadsComments:function(isbn){$("#goodReadsPlaceHolder").replaceWith("<iframe id='goodreads_iframe' class='goodReadsIFrame' src='https://www.goodreads.com/api/reviews_widget_iframe?did=DEVELOPER_ID&format=html&isbn="+isbn+"&links=660&review_back=fff&stars=000&text=000' width='100%' height='400px' frameborder='0'></iframe>")},loadEnrichmentInfo:function(id,forceReload){var url=Globals.path+"/GroupedWork/"+encodeURIComponent(id)+"/AJAX";var params="method=getEnrichmentInfo";if(forceReload!=undefined){params=params+"&reload=true"}var fullUrl=url+"?"+params;$.getJSON(fullUrl,function(data){try{var seriesData=data.seriesInfo;if(seriesData&&seriesData.titles.length>0){seriesScroller=new TitleScroller("titleScrollerSeries","Series","seriesList");$("#seriesInfo").show();seriesScroller.loadTitlesFromJsonData(seriesData);$("#seriesPanel").show()}else{$("#seriesPanel").hide()}var similarTitleData=data.similarTitles;if(similarTitleData&&similarTitleData.titles.length>0){morelikethisScroller=new TitleScroller("titleScrollerMoreLikeThis","MoreLikeThis","morelikethisList");$("#moreLikeThisInfo").show();morelikethisScroller.loadTitlesFromJsonData(similarTitleData)}var showGoDeeperData=data.showGoDeeper;if(showGoDeeperData){var goDeeperOptions=data.goDeeperOptions;for(var option in goDeeperOptions){if(option=="excerpt"){$("#excerpttab_label").show();$("#excerptPanel").show()}else if(option=="avSummary"){$("#tableofcontentstab_label").show();$("#avSummaryPlaceholder").show();$("#tableOfContentsPanel").show()}else if(option=="avSummary"||option=="tableOfContents"){$("#tableofcontentstab_label").show();$("#tableOfContentsPlaceholder").show();$("#tableOfContentsPanel").show()}}}if(VuFind.GroupedWork.hasTableOfContentsInRecord){$("#tableofcontentstab_label").show();$("#tableOfContentsPlaceholder").show();$("#tableOfContentsPanel").show()}var relatedContentData=data.relatedContent;if(relatedContentData&&relatedContentData.length>0){$("#relatedContentPlaceholder").html(relatedContentData)}var similarTitlesNovelist=data.similarTitlesNovelist;if(similarTitlesNovelist&&similarTitlesNovelist.length>0){$("#novelisttitlesPlaceholder").html(similarTitlesNovelist);$("#novelisttab_label").show();$("#similarTitlesPanel").show()}var similarAuthorsNovelist=data.similarAuthorsNovelist;if(similarAuthorsNovelist&&similarAuthorsNovelist.length>0){$("#novelistauthorsPlaceholder").html(similarAuthorsNovelist);$("#novelisttab_label").show();$("#similarAuthorsPanel").show()}var similarSeriesNovelist=data.similarSeriesNovelist;if(similarSeriesNovelist&&similarSeriesNovelist.length>0){$("#novelistseriesPlaceholder").html(similarSeriesNovelist);$("#novelisttab_label").show();$("#similarSeriesPanel").show()}}catch(e){alert("error loading enrichment: "+e)}})},loadReviewInfo:function(id){var url=Globals.path+"/GroupedWork/"+encodeURIComponent(id)+"/AJAX?method=getReviewInfo";$.getJSON(url,function(data){if(data.numSyndicatedReviews==0){$("#syndicatedReviewsPanel").hide()}else{var syndicatedReviewsData=data.syndicatedReviewsHtml;if(syndicatedReviewsData&&syndicatedReviewsData.length>0){$("#syndicatedReviewPlaceholder").html(syndicatedReviewsData)}}if(data.numEditorialReviews==0){$("#editorialReviewsPanel").hide()}else{var editorialReviewsData=data.editorialReviewsHtml;if(editorialReviewsData&&editorialReviewsData.length>0){$("#editorialReviewPlaceholder").html(editorialReviewsData)}}if(data.numCustomerReviews==0){$("#borrowerReviewsPanel").hide()}else{var customerReviewsData=data.customerReviewsHtml;if(customerReviewsData&&customerReviewsData.length>0){$("#customerReviewPlaceholder").html(customerReviewsData)}}})},markNotInterested:function(recordId){if(Globals.loggedIn){var url=Globals.path+"/GroupedWork/"+recordId+"/AJAX?method=markNotInterested";$.getJSON(url,function(data){if(data.result==true){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Sorry",data.message)}});return false}else{return VuFind.Account.ajaxLogin(null,function(){markNotInterested(source,recordId)},false)}},reloadCover:function(id){var url=Globals.path+"/GroupedWork/"+id+"/AJAX?method=reloadCover";$.getJSON(url,function(data){VuFind.showMessage("Success",data.message,true,true);setTimeout("VuFind.closeLightbox();",3e3)});return false},reloadEnrichment:function(id){VuFind.GroupedWork.loadEnrichmentInfo(id,true)},removeTag:function(id,tag){if(confirm('Are you sure you want to remove the tag "'+tag+'" from this title?')){var url=Globals.path+"/GroupedWork/"+id+"/AJAX?method=removeTag";url+="&tag="+encodeURIComponent(tag);$.getJSON(url,function(data){if(data.result==true){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Sorry",data.message)}});return false}return false},saveReview:function(id){if(!Globals.loggedIn){VuFind.Account.ajaxLogin(null,function(){this.saveReview(id)})}else{var comment=$("#comment"+id).val(),rating=$("#rating"+id).val(),url=Globals.path+"/GroupedWork/"+encodeURIComponent(id)+"/AJAX",params={method:"saveReview",comment:comment,rating:rating};$.getJSON(url,params,function(data){if(data.result){if(data.newReview){$("#customerReviewPlaceholder").append(data.reviewHtml)}else{$("#review_"+data.reviewId).replaceWith(data.reviewHtml)}VuFind.closeLightbox()}else{VuFind.showMessage("Error",data.message)}}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")})}return false},saveTag:function(id){var tag=$("#tags_to_apply").val();$("#saveToList-button").prop("disabled",true);var url=Globals.path+"/GroupedWork/"+id+"/AJAX";var params="method=SaveTag&"+"tag="+encodeURIComponent(tag);$.ajax({url:url+"?"+params,dataType:"json",success:function(data){if(data.success){VuFind.showMessage("Success",data.message);setTimeout("VuFind.closeLightbox();",3e3)}else{VuFind.showMessage("Error adding tags","There was an unexpected error adding tags to this title.<br/>"+data.message)}},error:function(jqXHR,textStatus){VuFind.showMessage("Error adding tags","There was an unexpected error adding tags to this title.<br/>"+textStatus)}})},saveToList:function(id){if(Globals.loggedIn){var listId=$("#addToList-list").val();var notes=$("#addToList-notes").val();var url=Globals.path+"/GroupedWork/"+encodeURIComponent(id)+"/AJAX";var params="method=saveToList&notes="+encodeURIComponent(notes)+"&listId="+encodeURIComponent(listId);$.getJSON(url+"?"+params,function(data){if(data.result){VuFind.showMessage("Added Successfully",data.message,2e3)}else{VuFind.showMessage("Error",data.message)}})}return false},sendEmail:function(id){if(Globals.loggedIn){var from=$("#from").val();var to=$("#to").val();var message=$("#message").val();var related_record=$("#related_record").val();var url=Globals.path+"/GroupedWork/"+encodeURIComponent(id)+"/AJAX";var params="method=sendEmail&from="+encodeURIComponent(from)+"&to="+encodeURIComponent(to)+"&message="+encodeURIComponent(message)+"&related_record="+encodeURIComponent(related_record);$.getJSON(url+"?"+params,function(data){if(data.result){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Error",data.message)}})}return false},sendSMS:function(id){if(Globals.loggedIn){var phoneNumber=$("#sms_phone_number").val();var provider=$("#provider").val();var related_record=$("#related_record").val();var url=Globals.path+"/GroupedWork/"+encodeURIComponent(id)+"/AJAX";var params="method=sendSMS&sms_phone_number="+encodeURIComponent(phoneNumber)+"&provider="+encodeURIComponent(provider)+"&related_record="+encodeURIComponent(related_record);$.getJSON(url+"?"+params,function(data){if(data.result){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Error",data.message)}})}return false},showEmailForm:function(trigger,id){var $trigger=$(trigger);if(Globals.loggedIn){var modalDialog=$("#modalDialog");$.getJSON(Globals.path+"/GroupedWork/"+id+"/AJAX?method=getEmailForm",function(data){$("#myModalLabel").html(data.title);$(".modal-body").html(data.modalBody);$(".modal-buttons").html(data.modalButtons)});modalDialog.load();modalDialog.modal("show")}else{VuFind.Account.ajaxLogin($trigger,function(){return VuFind.GroupedWork.showEmailForm(trigger,id)},false)}return false},showGroupedWorkInfo:function(id,browseCategoryId){var url=Globals.path+"/GroupedWork/AJAX?method=getWorkInfo&id="+id;if(browseCategoryId!=undefined){url+="&browseCategoryId="+browseCategoryId}VuFind.showMessage("Loading","Loading, please wait");$.getJSON(url,function(data){VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons)}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")});return false},showReviewForm:function(trigger,id){var $trigger=$(trigger);if(Globals.loggedIn){$.getJSON(Globals.path+"/GroupedWork/AJAX?method=getReviewForm&id="+id,function(data){VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons)}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")})}else{VuFind.Account.ajaxLogin($trigger,function(){return VuFind.GroupedWork.showReviewForm($trigger,id)},false)}return false},showSaveToListForm:function(trigger,id){if(Globals.loggedIn){VuFind.showMessage("Loading","Loading, please wait");var url=Globals.path+"/GroupedWork/"+id+"/AJAX?method=getSaveToListForm";$.getJSON(url,function(data){VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons)}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")})}else{trigger=$(trigger);VuFind.Account.ajaxLogin(trigger,function(){VuFind.GroupedWork.showSaveToListForm(trigger,id)})}return false},showSmsForm:function(trigger,id){var $trigger=$(trigger);if(Globals.loggedIn){var modalDialog=$("#modalDialog");$.getJSON(Globals.path+"/GroupedWork/"+id+"/AJAX?method=getSMSForm",function(data){$("#myModalLabel").html(data.title);$(".modal-body").html(data.modalBody);$(".modal-buttons").html(data.modalButtons)});modalDialog.load();modalDialog.modal("show")}else{VuFind.Account.ajaxLogin($trigger,function(){return VuFind.GroupedWork.showSmsForm(trigger,id)},false)}return false},showTagForm:function(trigger,id,source){if(Globals.loggedIn){var modalDialog=$("#modalDialog");$.getJSON(Globals.path+"/GroupedWork/"+id+"/AJAX?method=getAddTagForm",function(data){$("#myModalLabel").html(data.title);$(".modal-body").html(data.modalBody);$(".modal-buttons").html(data.modalButtons)});modalDialog.load();modalDialog.modal("show")}else{trigger=$(trigger);VuFind.Account.ajaxLogin(trigger,function(){VuFind.GroupedWork.showTagForm(trigger,id,source)},false)}return false}}}(VuFind.GroupedWork||{});
VuFind.Lists=function(){return{editListAction:function(){$("#listDescription,#listTitle,#FavEdit").hide();$("#listEditControls,#FavSave").show();return false},submitListForm:function(action){$("#myListActionHead").val(action);$("#myListFormHead").submit();return false},makeListPublicAction:function(){return this.submitListForm("makePublic")},makeListPrivateAction:function(){return this.submitListForm("makePrivate")},deleteListAction:function(){if(confirm("Are you sure you want to delete this list?")){this.submitListForm("deleteList")}return false},updateListAction:function(){return this.submitListForm("saveList")},deleteAllListItemsAction:function(){if(confirm("Are you sure you want to delete all titles from this list?  This cannot be undone.")){this.submitListForm("deleteAll")}return false},emailListAction:function(listId){var urlToDisplay=Globals.path+"/MyAccount/AJAX";VuFind.showMessage("Loading, please wait","...");$.getJSON(urlToDisplay,{method:"getEmailMyListForm",listId:listId},function(data){VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons)});return false},SendMyListEmail:function(){var url=Globals.path+"/MyAccount/AJAX";$.getJSON(url,{listId:$('#emailListForm input[name="listId"]').val(),to:$('#emailListForm input[name="to"]').val(),from:$('#emailListForm input[name="from"]').val(),message:$('#emailListForm textarea[name="message"]').val(),method:"sendMyListEmail"},function(data){if(data.result){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Error",data.message)}})},citeListAction:function(id){return VuFind.Account.ajaxLightbox("Citations for List Titles",Globals.path+"/MyAccount/AJAX?method=getCitationFormatsForm&listId="+id)},batchAddToListAction:function(id){return VuFind.Account.ajaxLightbox(Globals.path+"/MyAccount/AJAX/?method=getBulkAddToListForm&listId="+id)},processBulkAddForm:function(){$("#bulkAddToList").submit()},changeList:function(){var availableLists=$("#availableLists");window.location=Globals.path+"/MyAccount/MyList/"+availableLists.val()},printListAction:function(){window.print();return false},importListsFromClassic:function(){if(confirm("This will import any lists you had defined in the old catalog.  This may take several minutes depending on the size of your lists. Are you sure you want to continue?")){window.location=Globals.path+"/MyAccount/ImportListsFromClassic"}return false}}}(VuFind.Lists||{});
...
VuFind.MaterialsRequest=function(){return{getWorldCatIdentifiers:function(){var title=$("#title").val();var author=$("#author").val();var format=$("#format").val();if(title==""&&author==""){alert("Please enter a title and author before checking for an ISBN and OCLC Number")}else{var requestUrl=Globals.path+"/MaterialsRequest/AJAX?method=GetWorldCatIdentifiers&title="+encodeURIComponent(title)+"&author="+encodeURIComponent(author)+"&format="+encodeURIComponent(format);$.getJSON(requestUrl,function(data){if(data.success==true){var suggestedIdentifiers=$("#suggestedIdentifiers");suggestedIdentifiers.html(data.formattedSuggestions);suggestedIdentifiers.slideDown()}else{alert(data.error)}})}return false},cancelMaterialsRequest:function(id){if(confirm("Are you sure you want to cancel this request?")){var url=Globals.path+"/MaterialsRequest/AJAX?method=CancelRequest&id="+id;$.getJSON(url,function(data){if(data.success){alert("Your request was cancelled successfully.");window.location.reload()}else{alert(data.error)}});return false}else{return false}},showMaterialsRequestDetails:function(id){VuFind.Account.ajaxLightbox(Globals.path+"/MaterialsRequest/AJAX?method=MaterialsRequestDetails&id="+id,true)},updateMaterialsRequest:function(id){VuFind.Account.ajaxLightbox(Globals.path+"/MaterialsRequest/AJAX?method=UpdateMaterialsRequest&id="+id,true)},exportSelectedRequests:function(){var selectedRequests=this.getSelectedRequests(true);if(selectedRequests.length==0){return false}$("#updateRequests").submit();return true},updateSelectedRequests:function(){var newStatus=$("#newStatus").val();if(newStatus=="unselected"){alert("Please select a status to update the requests to.");return false}var selectedRequests=this.getSelectedRequests(false);if(selectedRequests.length!=0){$("#updateRequests").submit()}return false},getSelectedRequests:function(promptToSelectAll){var selectedRequests=$("input.select:checked").map(function(){return $(this).attr("name")+"="+$(this).val()}).get().join("&");if(selectedRequests.length==0){if(promptToSelectAll){var ret=confirm("You have not selected any requests, process all requests?");if(ret==true){selectedRequests=$("input.select").map(function(){return $(this).attr("name")+"=on"}).get().join("&");$(".select").attr("checked","checked")}}else{alert("Please select one or more requests to update")}}return selectedRequests},setIsbnAndOclcNumber:function(title,author,isbn,oclcNumber){$("#title").val(title);$("#author").val(author);$("#isbn").val(isbn);$("#oclcNumber").val(oclcNumber);$("#suggestedIdentifiers").slideUp()},setFieldVisibility:function(){$(".formatSpecificField").hide();var selectedFormat=$("#format").find("option:selected").val();$("."+selectedFormat+"Field").show();var author=$("#author");author.addClass("required");if(selectedFormat=="bluray"||selectedFormat=="dvd"||selectedFormat=="vhs"){$("#authorFieldLabel").html("Actor / Director:");author.removeClass("required")}else if(selectedFormat=="cdMusic"){$("#authorFieldLabel").html("Artist / Composer <span class='requiredIndicator'>*</span>")}else{$("#authorFieldLabel").html("Author <span class='requiredIndicator'>*</span>")}if(selectedFormat=="article"){$("#magazineTitle").addClass("required");$("#copyright").show();$("#acceptCopyrightYes").addClass("required");$("#supplementalDetails").hide();$("#titleLabel").html("Article Title <span class='requiredIndicator'>*</span>")}else{$("#magazineTitle").removeClass("required");$("#copyright").hide();$("#acceptCopyrightYes").removeClass("required");$("#supplementalDetails").show();$("#titleLabel").html("Title <span class='requiredIndicator'>*</span>")}if(selectedFormat=="ebook"||selectedFormat=="eaudio"){$("#illInfo").hide();$("#pickupLocationField").hide()}else{$("#illInfo").show();$("#pickupLocationField").show()}},updateHoldOptions:function(){var placeHold=$("input[name=placeHoldWhenAvailable]:checked").val();if(placeHold==1){$("#pickupLocationField").show();if($("#pickupLocation").find("option:selected").val()=="bookmobile"){$("#bookmobileStopField").show()}else{$("#bookmobileStopField").hide()}}else{$("#bookmobileStopField").hide();$("#pickupLocationField").hide()}},materialsRequestLogin:function(){var url=Globals.path+"/AJAX/JSON?method=loginUser";$.ajax({url:url,data:{username:$("#username").val(),password:$("#password").val()},success:function(response){if(response.result.success==true){$(".loginOptions").hide();$(".logoutOptions").show();$("#myAccountNameLink").html(response.result.name);if(response.result.enableMaterialsRequest==1){$("#materialsRequestLogin").hide();$(".materialsRequestLoggedInFields").show();if(response.result.phone){$("#phone").val(response.result.phone)}if(response.result.email){$("#email").val(response.result.email)}if(response.result.homeLocationId){var optionToSelect=$("#pickupLocation").find("option[value="+response.result.homeLocationId+"]");optionToSelect.attr("selected","selected")}}else{alert("Sorry, materials request functionality is only available to residents at this time.")}}else{alert("That login was not recognized.  Please try again.")}},error:function(){alert("That login was not recognized.  Please try again.")},dataType:"json",type:"post"});return false},printRequestBody:function(){$("#request_details_body").printElement()}}}(VuFind.MaterialsRequest||{});
VuFind.OverDrive=function(){return{cancelOverDriveHold:function(overdriveId){if(confirm("Are you sure you want to cancel this hold?")){var ajaxUrl=Globals.path+"/EcontentRecord/AJAX?method=CancelOverDriveHold&overDriveId="+overdriveId;$.ajax({url:ajaxUrl,cache:false,success:function(data){if(data.result){VuFind.showMessage("Hold Cancelled",data.message,true);$("#overDriveHold_"+overdriveId).hide()}else{VuFind.showMessage("Error Cancelling Hold",data.message,false)}},dataType:"json",async:false,error:function(){VuFind.showMessage("Error Cancelling Hold","An error occurred processing your request in OverDrive.  Please try again in a few minutes.",false)}})}return false},checkoutOverDriveItemOneClick:function(overdriveId){if(Globals.loggedIn){var ajaxUrl=Globals.path+"/EcontentRecord/AJAX?method=CheckoutOverDriveItem&overDriveId="+overdriveId;$.ajax({url:ajaxUrl,cache:false,success:function(data){if(data.result==true){VuFind.showMessage("Title Checked Out Successfully",data.message,true);window.location.href=Globals.path+"/MyAccount/CheckedOut"}else{if(data.noCopies==true){VuFind.closeLightbox();ret=confirm(data.message);if(ret==true){VuFind.OverDrive.placeOverDriveHold(overdriveId,null)}}else{VuFind.showMessage("Error Checking Out Title",data.message,false)}}},dataType:"json",async:false,error:function(){alert("An error occurred processing your request in OverDrive.  Please try again in a few minutes.");hideLightbox()}})}else{VuFind.Account.ajaxLogin(null,function(){VuFind.OverDrive.checkoutOverDriveItemOneClick(overdriveId)},false)}return false},doOverDriveHold:function(overDriveId,formatId,overdriveEmail,promptForOverdriveEmail){var url=Globals.path+"/EcontentRecord/AJAX?method=PlaceOverDriveHold&overDriveId="+overDriveId+"&formatId="+formatId+"&overdriveEmail="+overdriveEmail+"&promptForOverdriveEmail="+promptForOverdriveEmail;$.ajax({url:url,cache:false,success:function(data){if(data.availableForCheckout){VuFind.OverDrive.checkoutOverDriveItemOneClick(overdriveId)}else{VuFind.showMessage("Placed Hold",data.message,true)}},dataType:"json",async:false,error:function(){VuFind.showMessage("Error Placing Hold","An error occurred processing your request in OverDrive.  Please try again in a few minutes.",false)}})},followOverDriveDownloadLink:function(overDriveId,formatId){var ajaxUrl=Globals.path+"/EcontentRecord/AJAX?method=GetDownloadLink&overDriveId="+overDriveId+"&formatId="+formatId;$.ajax({url:ajaxUrl,cache:false,success:function(data){if(data.result){window.location.href=data.downloadUrl}else{alert(data.message)}},dataType:"json",async:false,error:function(){alert("An error occurred processing your request in OverDrive.  Please try again in a few minutes.");hideLightbox()}})},getOverDriveHoldPrompts:function(overDriveId,formatId){var url=Globals.path+"/OverDrive/"+overDriveId+"/AJAX?method=GetOverDriveHoldPrompts";if(formatId!=undefined){url+="&formatId="+formatId}var result=true;$.ajax({url:url,cache:false,success:function(data){result=data;if(data.promptNeeded){VuFind.showMessageWithButtons(data.promptTitle,data.prompts,data.buttons)}},dataType:"json",async:false,error:function(){alert("An error occurred processing your request in OverDrive.  Please try again in a few minutes.");VuFind.closeLightbox()}});return result},placeOverDriveHold:function(overDriveId,formatId){if(Globals.loggedIn){var promptInfo=VuFind.OverDrive.getOverDriveHoldPrompts(overDriveId,formatId,"hold");if(!promptInfo.promptNeeded){VuFind.OverDrive.doOverDriveHold(overDriveId,formatId,promptInfo.overdriveEmail,promptInfo.promptForOverdriveEmail)}}else{VuFind.Account.ajaxLogin(null,function(){VuFind.OverDrive.placeOverDriveHold(overDriveId,formatId)})}return false},processOverDriveHoldPrompts:function(){var overdriveHoldPromptsForm=$("#overdriveHoldPromptsForm");var overdriveId=overdriveHoldPromptsForm.find("input[name=overdriveId]").val();var formatId=-1;if(overdriveHoldPromptsForm.find("input[name=formatId]")&&overdriveHoldPromptsForm.find("input[name=formatId]").val()!=undefined){formatId=overdriveHoldPromptsForm.find("input[name=formatId]").val();if(formatId==undefined){formatId=""}}else if($("#formatId").find(":selected")&&$("#formatId").find(":selected").val()!=undefined){formatId=$("#formatId").find(":selected").val()}var promptForOverdriveEmail;if(overdriveHoldPromptsForm.find("input[name=promptForOverdriveEmail]").is(":checked")){promptForOverdriveEmail=0}else{promptForOverdriveEmail=1}var overdriveEmail=overdriveHoldPromptsForm.find("input[name=overdriveEmail]").val();VuFind.OverDrive.doOverDriveHold(overdriveId,formatId,overdriveEmail,promptForOverdriveEmail)},returnOverDriveTitle:function(overDriveId,transactionId){if(confirm("Are you sure you want to return this title?")){VuFind.showMessage("Returning Title","Returning your title in OverDrive.  This may take a minute.");var ajaxUrl=Globals.path+"/EcontentRecord/AJAX?method=ReturnOverDriveItem&overDriveId="+overDriveId+"&transactionId="+transactionId;$.ajax({url:ajaxUrl,cache:false,success:function(data){VuFind.showMessage("Title Returned",data.message);if(data.result){setTimeout(function(){VuFind.closeLightbox();window.location.href=window.location.href},3e3)}},dataType:"json",async:false,error:function(){VuFind.showMessage("Error Returning Title","An error occurred processing your request in OverDrive.  Please try again in a few minutes.")}})}return false},selectOverDriveDownloadFormat:function(overDriveId){var selectedOption=$("#downloadFormat_"+overDriveId+" option:selected");var selectedFormatId=selectedOption.val();var selectedFormatText=selectedOption.text();if(selectedFormatId==-1){alert("Please select a format to download.")}else{if(confirm("Are you sure you want to download the "+selectedFormatText+" format? You cannot change format after downloading.")){var ajaxUrl=Globals.path+"/EcontentRecord/AJAX?method=SelectOverDriveDownloadFormat&overDriveId="+overDriveId+"&formatId="+selectedFormatId;$.ajax({url:ajaxUrl,cache:false,success:function(data){if(data.result){window.location.href=data.downloadUrl}else{VuFind.showMessage("Error Selecting Format",data.message)}},dataType:"json",async:false,error:function(){VuFind.showMessage("Error Selecting Format","An error occurred processing your request in OverDrive.  Please try again in a few minutes.")}})}}return false}}}(VuFind.OverDrive||{});
VuFind.Prospector=function(){return{getProspectorResults:function(prospectorNumTitlesToLoad,prospectorSavedSearchId){var url=Globals.path+"/Search/AJAX";var params="method=getProspectorResults&prospectorNumTitlesToLoad="+encodeURIComponent(prospectorNumTitlesToLoad)+"&prospectorSavedSearchId="+encodeURIComponent(prospectorSavedSearchId);var fullUrl=url+"?"+params;$.ajax({url:fullUrl,success:function(data){var prospectorSearchResults=$(data).find("ProspectorSearchResults").text();if(prospectorSearchResults){if(prospectorSearchResults.length>0){$("#prospectorSearchResultsPlaceholder").html(prospectorSearchResults)}}}})},loadRelatedProspectorTitles:function(id){var url;url=Globals.path+"/GroupedWork/"+encodeURIComponent(id)+"/AJAX";var params="method=getProspectorInfo";var fullUrl=url+"?"+params;$.getJSON(fullUrl,function(data){if(data.numTitles==0){$("#prospectorPanel").hide()}else{$("#inProspectorPlaceholder").html(data.formattedData)}})}}}(VuFind.Prospector||{});
VuFind.Ratings=function(){$(document).ready(function(){VuFind.Ratings.initializeRaters()});return{initializeRaters:function(){$(".rater").each(function(){var ratingElement=$(this),userRating=ratingElement.data("user_rating"),id=ratingElement.data("id"),options={id:id,rating:parseFloat(userRating>0?userRating:ratingElement.data("average_rating")),url:Globals.path+"/GroupedWork/AJAX"};ratingElement.rater(options)})},doRatingReview:function(id){$.getJSON(Globals.path+"/GroupedWork/"+id+"/AJAX?method=getPromptforReviewForm",function(data){if(data.prompt)VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons);if(data.error)VuFind.showMessage("Error",data.message)})},doNoRatingReviews:function(){$.getJSON(Globals.path+"/GroupedWork/AJAX?method=setNoMoreReviews",function(data){if(data.success)VuFind.showMessage("Success","You will no longer be asked to give a review.",true);else VuFind.showMessage("Error","Failed to save your setting.")}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")})}}}(VuFind.Ratings);$.fn.rater=function(options){var opts=$.extend({},$.fn.rater.defaults,options);return this.each(function(){var $this=$(this),$on=$this.find(".ui-rater-starsOn"),$off=$this.find(".ui-rater-starsOff");if(opts.size==undefined)opts.size=$off.height();if(opts.rating==undefined){opts.rating=$on.width()/$off.width()}else{$on.width($off.width()*(opts.rating/opts.ratings.length))}if(opts.id==undefined)opts.id=$this.attr("id");var initialRating=opts.rating;if(!$this.hasClass("ui-rater-bindings-done")){$this.addClass("ui-rater-bindings-done");$off.mousemove(function(e){var left=e.clientX-$off.offset().left,width=$off.width()-($off.width()-left);width=Math.min(Math.ceil(width/(opts.size/opts.step))*opts.size/opts.step,opts.size*opts.ratings.length);$on.width(width);var r=Math.round($on.width()/$off.width()*(opts.ratings.length*opts.step))/opts.step;$this.attr("title",'Click to Rate "'+r+' stars"')}).hover(function(e){$on.addClass("ui-rater-starsHover")},function(e){$on.removeClass("ui-rater-starsHover");$on.width(initialRating*opts.size)}).click(function(e){var r=Math.round($on.width()/$off.width()*(opts.ratings.length*opts.step))/opts.step;$.fn.rater.rate($this,opts,r)}).css("cursor","pointer");$on.css("cursor","pointer")}})};$.fn.rater.defaults={url:location.href,ratings:["Hated It","Didn't Like It","Liked It","Really Liked It","Loved It"],step:1};$.fn.rater.rate=function($this,opts,rating){if(Globals.loggedIn){var $on=$this.find(".ui-rater-starsOn"),$off=$this.find(".ui-rater-starsOff");$off.fadeTo(600,.4,function(){$.getJSON(opts.url,{method:"RateTitle",id:opts.id,rating:rating},function(data){if(data.error){VuFind.showMessage("Error",data.error);$off.fadeTo(500,1).mouseleave()}if(data.rating){opts.rating=data.rating;$off.fadeTo(600,.1,function(){$on.removeClass("ui-rater-starsHover").width(opts.rating*opts.size).addClass("userRated");$off.fadeTo(500,1);$this.attr("title","Your rating: "+rating.toFixed(1));if($this.data("show_review")==true){VuFind.Ratings.doRatingReview(opts.id)}})}}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.");$off.fadeTo(500,1).mouseleave()})})}else{VuFind.Account.ajaxLogin(null,function(){$.fn.rater.rate($this,opts,rating)})}};
VuFind.Account.ReadingHistory=function(){return{deletedMarkedAction:function(){if(confirm("The marked items will be irreversibly deleted from your reading history.  Proceed?")){$("#readingHistoryAction").val("deleteMarked");$("#readingListForm").submit()}return false},deleteAllAction:function(){if(confirm("Your entire reading history will be irreversibly deleted.  Proceed?")){$("#readingHistoryAction").val("deleteAll");$("#readingListForm").submit()}return false},optOutAction:function(showError){if(showError){alert("Your reading history must be deleted before you can Opt Out.");return false}$("#readingHistoryAction").val("optOut");$("#readingListForm").submit();return false},optInAction:function(){$("#readingHistoryAction").val("optIn");$("#readingListForm").submit();return false},exportListAction:function(){$("#readingHistoryAction").val("exportToExcel");$("#readingListForm").submit();return false}}}(VuFind.Account.ReadingHistory||{});
VuFind.Record=function(){return{GetGoDeeperData:function(id,isbn,upc,dataType){var placeholder;if(dataType=="excerpt"){placeholder=$("#excerptPlaceholder")}else if(dataType=="avSummary"){placeholder=$("#avSummaryPlaceholder")}else if(dataType=="tableOfContents"){placeholder=$("#tableOfContentsPlaceholder")}if(placeholder.hasClass("loaded"))return;placeholder.show();var url=Globals.path+"/Record/"+encodeURIComponent(id)+"/AJAX";var params="method=GetGoDeeperData&dataType="+encodeURIComponent(dataType)+"&isbn="+encodeURIComponent(isbn)+"&upc="+encodeURIComponent(upc);var fullUrl=url+"?"+params;$.ajax({url:fullUrl,success:function(data){placeholder.html(data);placeholder.addClass("loaded")},failure:function(){alert("Error: Could Not Load Syndetics information.")}})},loadEnrichmentInfo:function(id,isbn,upc,source){var url;if(source=="VuFind"){url=Globals.path+"/Record/"+encodeURIComponent(id)+"/AJAX"}else{url=Globals.path+"/EcontentRecord/"+encodeURIComponent(id)+"/AJAX"}var params="method=GetEnrichmentInfoJSON&isbn="+encodeURIComponent(isbn)+"&upc="+encodeURIComponent(upc);var fullUrl=url+"?"+params;$.ajax({url:fullUrl,dataType:"json",success:function(data){try{var seriesData=data.seriesInfo;if(seriesData&&seriesData.length>0){var seriesScroller=new TitleScroller("titleScrollerSeries","Series","seriesList");seriesData=$.parseJSON(seriesData);if(seriesData.titles.length>0){$("#list-series-tab").show();$("#relatedTitleInfo").show();seriesScroller.loadTitlesFromJsonData(seriesData)}}var showGoDeeperData=data.showGoDeeper;if(showGoDeeperData){var goDeeperOptions=data.goDeeperOptions;for(option in goDeeperOptions){if(option=="excerpt"){$("#excerpttab_label").show()}else if(option=="avSummary"){$("#tableofcontentstab_label").show();$("avSummaryPlaceholder").show()}else if(option=="avSummary"||option=="tableOfContents"){$("#tableofcontentstab_label").show();$("tableOfContentsPlaceholder").show()}}}var relatedContentData=data.relatedContent;if(relatedContentData&&relatedContentData.length>0){$("#relatedContentPlaceholder").html(relatedContentData)}}catch(e){alert("error loading enrichment: "+e)}},failure:function(){alert("Error: Could Not Load Enrichment information.")}})},loadHoldingsInfo:function(module,id,shortId,source){var url;if(source=="VuFind"){url=Globals.path+"/"+module+"/"+encodeURIComponent(id)+"/AJAX"}else{url=Globals.path+"/EcontentRecord/"+encodeURIComponent(id)+"/AJAX"}var params="method=GetHoldingsInfo";var fullUrl=url+"?"+params;$.ajax({url:fullUrl,success:function(data){if(source=="VuFind"){var holdingsData=$(data).find("Holdings").text();if(holdingsData){if(holdingsData.length>0){if(holdingsData.match(/No Copies Found/i)){try{if($("#prospectortab_label").is(":visible")){$("#moredetails-tabs").tabs("option","active",1)}else{$("#moredetails-tabs").tabs("option","active",2)}$("#holdingstab_label").hide()}catch(e){}}else{$("#holdingsPlaceholder").html(holdingsData)}}}var summaryDetails=$(data).find("SummaryDetails");var callNumber=summaryDetails.find("callnumber").text();if(callNumber.length>0){$("#callNumberValue").html(callNumber)}else{$("#callNumberRow").hide()}var location=summaryDetails.find("availableAt").text();if(location.length>0){$("#locationValue").html(location)}else{location=summaryDetails.find("location").text();if(location.length>0){$("#locationValue").html(location)}else{$("#locationRow").hide()}}var status=summaryDetails.find("status").text();if(status=="Available At"){status="Available"}else if(status=="Marmot"){status="Available from another library"}$("#statusValue").html(status).addClass(summaryDetails.find("class").text())}else{var formatsData=$(data).find("Formats").text();if(formatsData){if(formatsData.length>0){$("#formatsPlaceholder").html(formatsData).trigger("create")}else{$("#formatsPlaceholder").html("No Formats Information found, please try again later.")}}var copiesData=$(data).find("Copies").text();var formatsTabLink=$("#formatstabLink").find("a");if(copiesData){if(copiesData.length>0){$("#copiesPlaceholder").html(copiesData).trigger("create")}else{$("#copiestabLink").hide();$("#copiesPlaceholder").html("No Copies Information found, please try again later.");formatsTabLink.text("Copies")}}else{$("#copiestabLink").hide();$("#copiesPlaceholder").html("No Copies Information found, please try again later.");formatsTabLink.text("Copies")}}var holdingsSummary=$(data).find("HoldingsSummary").text();if(holdingsSummary){if(holdingsSummary.length>0){$("#holdingsSummaryPlaceholder").html(holdingsSummary)}}var showPlaceHold=$(data).find("ShowPlaceHold").text();if(showPlaceHold){if(showPlaceHold.length>0&&showPlaceHold==1){$("#placeHold"+shortId).show()}}var showCheckout=$(data).find("ShowCheckout").text();if(showCheckout){if(showCheckout.length>0&&showCheckout==1){$("#checkout"+shortId).show()}}var showBookMaterial=$(data).find("showBookMaterial").text();if(showBookMaterial){if(showBookMaterial.length>0&&showBookMaterial==1){$("#bookMaterialButton").show()}}var showAccessOnline=$(data).find("ShowAccessOnline").text();if(showAccessOnline){if(showAccessOnline.length>0&&showAccessOnline==1){if($(data).find("AccessOnlineUrl").length>0){var url=$(data).find("AccessOnlineUrl").text();var text=$(data).find("AccessOnlineText").text();var accessOnlineLink=$("#accessOnline"+id);accessOnlineLink.attr("href",url);accessOnlineLink.text($("<div/>").html(text).text())}$(".accessOnlineLink").show()}}var showAddToWishList=$(data).find("ShowAddToWishlist").text();if(showAddToWishList){if(showAddToWishList.length>0&&showAddToWishList==1){$("#addToWishList"+id).show()}}}})},loadReviewInfo:function(id,isbn,source){var url;if(source=="VuFind"){url=Globals.path+"/Record/"+encodeURIComponent(id)+"/AJAX"}else{url=Globals.path+"/EcontentRecord/"+encodeURIComponent(id)+"/AJAX"}var params="method=GetReviewInfo&isbn="+encodeURIComponent(isbn);var fullUrl=url+"?"+params;$.ajax({url:fullUrl,success:function(data){var reviewsData=$(data).find("Reviews").text();if(reviewsData&&reviewsData.length>0){$("#reviewPlaceholder").html(reviewsData)}}})},saveReview:function(id,shortId){if(Globals.loggedIn){if(shortId==null||shortId==""){shortId=id}var comment=$("#comment"+shortId).val();var url=Globals.path+"/Record/"+encodeURIComponent(id)+"/AJAX";var params="method=SaveComment&comment="+encodeURIComponent(comment);$.ajax({url:url+"?"+params,dataType:"json",success:function(data){var result=false;if(data){result=data.result}if(result&&result.length>0){if(result=="Done"){$("#comment"+shortId).val("");if($("#commentList").length>0){LoadComments(id)}else{alert("Thank you for your review.");VuFind.closeLightbox()}}else{alert("Error: Your review was not saved successfully")}}else{alert("Error: Your review was not saved successfully")}},error:function(){alert("Unable to save your comment.")}})}return false},sendEmail:function(id,source){var emailForm=$("#emailForm");var to=emailForm.find("input[name=to]").val();var from=emailForm.find("input[name=from]").val();var message=emailForm.find("input[name=message]").val();var url;if(source=="VuFind"){url=Globals.path+"/Record/"+encodeURIComponent(id)+"/AJAX"}else{url=Globals.path+"/EcontentRecord/"+encodeURIComponent(id)+"/AJAX"}var params="method=SendEmail&"+"to="+encodeURIComponent(to)+"&"+"from="+encodeURIComponent(from)+"&"+"message="+encodeURIComponent(message);$.ajax({url:url+"?"+params,success:function(data){var value=$(data).find("result");if(value){if(value.text()=="Done"){$(".modal-body").html("<div class='alert alert-success'>Your e-mail has been sent</div>");setTimeout("VuFind.closeLightbox();",3e3)}else{$(".modal-body").html("<div class='alert alert-error'>Could not send e-mail</div>")}}else{$(".modal-body").html("<div class='alert alert-error'>Failed to send e-mail</div>")}},error:function(){$(".modal-body").html("<div class='alert alert-error'>Unexpected error sending e-mail</div>")}})},sendSMS:function(id,source){var smsForm=$("#smsForm");var to=smsForm.find("input[name=to]").val();var provider=smsForm.find("input[name=provider]").val();var url;if(source=="VuFind"){url=Globals.path+"/Record/"+encodeURIComponent(id)+"/AJAX"}else{url=Globals.path+"/EcontentRecord/"+encodeURIComponent(id)+"/AJAX"}var params="method=SendSMS&"+"to="+encodeURIComponent(to)+"&"+"provider="+encodeURIComponent(provider);$.ajax({url:url+"?"+params,success:function(data){var value=$(data).find("result");if(value){if(value.text()=="Done"){$(".modal-body").html("<div class='alert alert-success'>Your Text Message has been sent</div>");setTimeout("VuFind.closeLightbox();",3e3)}else{$(".modal-body").html("<div class='alert alert-error'>Could not send text message</div>")}}else{$(".modal-body").html("<div class='alert alert-error'>Failed to send text message</div>")}},error:function(){$(".modal-body").html("<div class='alert alert-error'>Unexpected error sending text message</div>")}})},showPlaceHold:function(module,id){if(Globals.loggedIn){var source;if(id.indexOf(":")>0){var idParts=id.split(":",2);source=idParts[0];id=idParts[1]}else{source="ils"}var url=Globals.path+"/"+module+"/"+id+"/AJAX?method=getPlaceHoldForm&recordSource="+source;$.getJSON(url,function(data){VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons)}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")})}else{VuFind.Account.ajaxLogin(null,function(){VuFind.Record.showPlaceHold(module,id)},false)}return false},showBookMaterial:function(id){if(Globals.loggedIn){VuFind.loadingMessage();if(id.indexOf(":")>0){var idParts=id.split(":",2);id=idParts[1]}$.getJSON(Globals.path+"/Record/"+id+"/AJAX?method=getBookMaterialForm",function(data){VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons)}).fail(VuFind.ajaxFail)}else{VuFind.Account.ajaxLogin(null,function(){VuFind.Record.showBookMaterial(id)},false)}return false},submitBookMaterialForm:function(){var params=$("#bookMaterialForm").serialize();VuFind.showMessage("Booking","Processing, please wait.");$.getJSON(Globals.path+"/Record/AJAX",params+"&method=bookMaterial",function(data){if(data.modalBody)VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons);if(data.success)VuFind.showMessage("Success",data.message);else if(data.message)VuFind.showMessage("Error",data.message)}).fail(VuFind.ajaxFail)},submitHoldForm:function(){var id=$("#id").val(),autoLogOut=$("#autologout").prop("checked"),selectedItem=$("#selectedItem"),module=$("#module").val(),params={method:"placeHold",campus:$("#campus").val(),cancelHoldDate:$("#cancelHoldDate").text(),recordSource:$("#recordSource").val(),account:$("#account").val()};if(autoLogOut){params["autologout"]=true}if(selectedItem.length>0){params["selectedItem"]=selectedItem.val()}$.getJSON(Globals.path+"/"+module+"/"+id+"/AJAX",params,function(data){if(data.success){if(data.needsItemLevelHold){$(".modal-body").html(data.message)}else{VuFind.showMessage("Hold Placed Successfully",data.message,false,autoLogOut)}}else{VuFind.showMessage("Hold Failed",data.message,false,autoLogOut)}}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")})},reloadCover:function(id){var url=Globals.path+"/Record/"+id+"/AJAX?method=reloadCover";$.getJSON(url,function(data){VuFind.showMessage("Success",data.message,true,true);setTimeout("VuFind.closeLightbox();",3e3)}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")});return false},moreContributors:function(){document.getElementById("showAdditionalContributorsLink").style.display="none";document.getElementById("additionalContributors").style.display="block"},lessContributors:function(){document.getElementById("showAdditionalContributorsLink").style.display="block";document.getElementById("additionalContributors").style.display="none"}}}(VuFind.Record||{});
VuFind.Responsive=function(){$(document).ready(function(){$(window).resize(function(){VuFind.Responsive.adjustLayout()}).trigger("resize");$("#lookfor").on("keyup",function(event){$(this).height(0);if(this.scrollHeight<32){$(this).height(18)}else{$(this).height(this.scrollHeight)}}).on("keydown",function(event){if(event.which==13){event.preventDefault();$("#searchForm").submit()}}).keyup()});try{var mediaQueryList=window.matchMedia("print");mediaQueryList.addListener(function(mql){VuFind.Responsive.isPrint=mql.matches;VuFind.Responsive.adjustLayout()})}catch(err){}window.onbeforeprint=function(){VuFind.Responsive.isPrint=true;VuFind.Responsive.adjustLayout()};return{adjustLayout:function(){var resolution=document.documentElement.clientWidth;var mainContentElement=$("#main-content-with-sidebar");var xsContentInsertionPointElement=$("#xs-main-content-insertion-point");var mainContent;if(resolution<768&&!VuFind.Responsive.isPrint){mainContent=mainContentElement.html();if(mainContent&&mainContent.length){xsContentInsertionPointElement.html(mainContent);mainContentElement.html("");VuFind.initCarousels()}}else{mainContent=xsContentInsertionPointElement.html();if(mainContent&&mainContent.length){mainContentElement.html(mainContent);xsContentInsertionPointElement.html("");VuFind.initCarousels()}}}}}(VuFind.Responsive||{});
VuFind.ResultsList=function(){return{statusList:[],seriesList:[],addIdToSeriesList:function(isbn){this.seriesList[this.seriesList.length]=isbn},addIdToStatusList:function(id,type,useUnscopedHoldingsSummary){if(type==undefined){type="VuFind"}var idVal=[];idVal["id"]=id;idVal["useUnscopedHoldingsSummary"]=useUnscopedHoldingsSummary;idVal["type"]=type;this.statusList[this.statusList.length]=idVal},initializeDescriptions:function(){$(".descriptionTrigger").each(function(){var descElement=$(this);var descriptionContentClass=descElement.data("content_class");options={html:true,trigger:"hover",title:"Description",content:VuFind.ResultsList.loadDescription(descriptionContentClass)};descElement.popover(options)})},lessFacets:function(name){document.getElementById("more"+name).style.display="block";document.getElementById("narrowGroupHidden_"+name).style.display="none"},loadDescription:function(descriptionContentClass){var contentHolder=$(descriptionContentClass);return contentHolder[0].innerHTML},loadStatusSummaries:function(){var now=new Date;var ts=Date.UTC(now.getFullYear(),now.getMonth(),now.getDay(),now.getHours(),now.getMinutes(),now.getSeconds(),now.getMilliseconds());var callGetEContentStatusSummaries=false;var eContentUrl=Globals.path+"/Search/AJAX?method=GetEContentStatusSummaries";for(var j=0;j<this.statusList.length;j++){if(this.statusList[j]["type"]=="eContent"){eContentUrl+="&id[]="+encodeURIComponent(this.statusList[j]["id"]);if(this.statusList[j]["useUnscopedHoldingsSummary"]){eContentUrl+="&useUnscopedHoldingsSummary=true"}callGetEContentStatusSummaries=true}else if(this.statusList[j]["type"]=="VuFind"){var url=Globals.path+"/Search/AJAX?method=GetStatusSummaries";url+="&id[]="+encodeURIComponent(this.statusList[j]["id"]);if(this.statusList[j]["useUnscopedHoldingsSummary"]){url+="&useUnscopedHoldingsSummary=true"}url+="&time="+ts;$.getJSON(url,function(data){var items=data.items;var elemId;var showPlaceHold;var numHoldable=0;for(var i=0;i<items.length;i++){try{elemId=items[i].shortId;if(items[i].showPlaceHold==null){showPlaceHold=0}else{showPlaceHold=items[i].showPlaceHold}if(showPlaceHold=="1"||showPlaceHold==true){numHoldable++;var placeHoldButton=$("#placeHold"+elemId);if(placeHoldButton.length>0){placeHoldButton.show()}}var holdingsSummaryId="#holdingsSummary"+elemId;var holdingSum=$(holdingsSummaryId);if(holdingSum.length>0){divClass=items[i]["class"];holdingSum.addClass(divClass);var formattedHoldingsSummary=items[i].formattedHoldingsSummary;holdingSum.replaceWith(formattedHoldingsSummary)}var callNumberSpan=$("#callNumberValue"+elemId);if(callNumberSpan.length>0){var callNumber=items[i].callnumber;if(callNumber){callNumberSpan.html(callNumber)}else{callNumberSpan.html("N/A")}}var locationSpan=$("#locationValue"+elemId);if(locationSpan.length>0){var availableAt=items[i].availableAt;if(availableAt){locationSpan.html(availableAt)}else{var location=items[i].location;if(location){locationSpan.html(location)}else{locationSpan.html("N/A")}}}var statusSpan=$("#statusValue"+elemId);var statusClass=undefined;if(statusSpan.length>0){var status=items[i].statusText;if(status){if(status=="Available At"){status="Available"}statusSpan.html(status)}else{statusSpan.html("Unknown")}statusClass=items[i]["class"];if(statusClass){statusSpan.addClass(statusClass)}}var copiesSpan=$("#copiesValue"+elemId);if(copiesSpan.length>0){var copies=items[i].copies;if(copies){copiesSpan.html(copies)}else{copies.html("No copies found")}statusClass=items[i]["class"];if(statusClass){statusSpan.addClass(statusClass)}}var downloadLinkSpan=$("#downloadLinkValue"+elemId);if(downloadLinkSpan.length>0){var isDownloadable=items[i].isDownloadable;if(isDownloadable==1){var downloadLink=items[i].downloadLink;var downloadText=items[i].downloadText;$("#downloadLinkValue"+elemId).html("<a href='"+decodeURIComponent(downloadLink)+"'>"+downloadText+"</a>");$("#downloadLink"+elemId).show()}}$("#holdingsSummary"+elemId).addClass("loaded")}catch(err){}}if(numHoldable>0){$(".requestSelectedItems").show()}}).error(function(jqXHR,textStatus,errorThrown){})}else{var overDriveUrl=Globals.path+"/Search/AJAX?method=GetEContentStatusSummaries";overDriveUrl+="&id[]="+encodeURIComponent(this.statusList[j]["id"]);$.ajax({url:overDriveUrl,success:function(data){var items=$(data).find("item");$(items).each(function(index,item){var elemId=$(item).attr("id");var holdingsSummaryId="#holdingsEContentSummary"+elemId;$(holdingsSummaryId).replaceWith($(item).find("formattedHoldingsSummary").text());if($(item).find("showplacehold").text()==1){$("#placeEcontentHold"+elemId).show()}else if($(item).find("showcheckout").text()==1){$("#checkout"+elemId).show()}else if($(item).find("showaccessonline").text()==1){$("#accessOnline"+elemId).show()}else if($(item).find("showaddtowishlist").text()==1){$("#addToWishList"+elemId).show()}var statusId="#statusValue"+elemId;if($(statusId).length>0){var status=$(item).find("status").text();$(statusId).text(status);var statusClass=$(item).find("class").text();if(statusClass){$(statusId).addClass(statusClass)}}$(holdingsSummaryId).addClass("loaded")})}})}}eContentUrl+="&time="+ts;if(callGetEContentStatusSummaries){$.ajax({url:eContentUrl,success:function(data){var items=$(data).find("item");$(items).each(function(index,item){var elemId=$(item).attr("id");var eContentElementId="#holdingsEContentSummary"+elemId;$(eContentElementId).replaceWith($(item).find("formattedHoldingsSummary").text());if($(item).find("showplacehold").text()==1){$("#placeEcontentHold"+elemId).show()}else if($(item).find("showcheckout").text()==1){$("#checkout"+elemId).show()}else if($(item).find("showaccessonline").text()==1){if($(item).find("accessonlineurl").length>0){var url=$(item).find("accessonlineurl").text();var text=$(item).find("accessonlinetext").text();$("#accessOnline"+elemId+" a").attr("href",url).text($("<div/>").html(text).text())}$("#accessOnline"+elemId).show()}else if($(item).find("showaddtowishlist").text()==1){$("#addToWishList"+elemId).show()}var statusId="#statusValue"+elemId;if($(statusId).length>0){var status=$(item).find("status").text();$(statusId).text(status);var statusClass=$(item).find("class").text();if(statusClass){$("#statusValue"+elemId).addClass(statusClass)}}$("#holdingsEContentSummary"+elemId).addClass("loaded")})}})}this.statusList=[]},moreFacets:function(name){document.getElementById("more"+name).style.display="none";document.getElementById("narrowGroupHidden_"+name).style.display="block"},moreFacetPopup:function(title,name){VuFind.showMessage(title,$("#moreFacetPopup_"+name).html())},toggleFacetVisibility:function(){$facetsSection=$("#collapse-side-facets")},toggleRelatedManifestations:function(manifestationId){$("#relatedRecordPopup_"+manifestationId).toggleClass("hidden");var manifestationToggle=$("#manifestation-toggle-"+manifestationId);manifestationToggle.toggleClass("collapsed");if(manifestationToggle.hasClass("collapsed")){manifestationToggle.html("+")}else{manifestationToggle.html("-")}return false}}}(VuFind.ResultsList||{});
VuFind.Searches=function(){$(document).ready(function(){VuFind.Searches.enableSearchTypes();VuFind.Searches.initAutoComplete()});return{searchGroups:[],curPage:1,displayMode:"list",displayModeClasses:{covers:"home-page-browse-thumbnails",list:""},getPreferredDisplayMode:function(){if(!Globals.opac&&VuFind.hasLocalStorage()){temp=window.localStorage.getItem("searchResultsDisplayMode");if(VuFind.Searches.displayModeClasses.hasOwnProperty(temp)){VuFind.Searches.displayMode=temp;$('input[name="view"]',"#searchForm").val(VuFind.Searches.displayMode)}}},toggleDisplayMode:function(selectedMode){var mode=this.displayModeClasses.hasOwnProperty(selectedMode)?selectedMode:this.displayMode,searchBoxView=$('input[name="view"]',"#searchForm"),paramString=this.replaceQueryParam("page","",this.replaceQueryParam("view",mode));this.displayMode=mode;this.curPage=1;if(searchBoxView)searchBoxView.val(this.displayMode);if(!Globals.opac&&VuFind.hasLocalStorage()){window.localStorage.setItem("searchResultsDisplayMode",this.displayMode)}location.replace(location.pathname+paramString)},replaceQueryParam:function(param,newValue,search){if(typeof search=="undefined")search=location.search;var regex=new RegExp("([?;&])"+param+"[^&;]*[;&]?"),query=search.replace(regex,"$1").replace(/&$/,"");return newValue?(query.length>2?query+"&":"?")+param+"="+newValue:query},getMoreResults:function(){var url=Globals.path+"/Search/AJAX",params=this.replaceQueryParam("page",this.curPage+1)+"&method=getMoreSearchResults",divClass=this.displayModeClasses[this.displayMode];params=this.replaceQueryParam("view",this.displayMode,params);if(params.search(/[?;&]replacementTerm=/)!=-1){var searchTerm=location.search.split("replacementTerm=")[1].split("&")[0];params=this.replaceQueryParam("lookfor",searchTerm,params)}$.getJSON(url+params,function(data){if(data.success==false){VuFind.showMessage("Error loading search information","Sorry, we were not able to retrieve additional results.")}else{var newDiv=$(data.records).hide();$("."+divClass).filter(":last").after(newDiv);newDiv.fadeIn("slow");if(data.lastPage)$("#more-browse-results").hide();else VuFind.Searches.curPage++}}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")});return false},initAutoComplete:function(){try{$("#lookfor").autocomplete({source:function(request,response){var url=Globals.path+"/Search/AJAX?method=GetAutoSuggestList&searchTerm="+$("#lookfor").val();$.ajax({url:url,dataType:"json",success:function(data){response(data)}})},position:{my:"left top",at:"left bottom",of:"#lookfor",collision:"none"},minLength:4,delay:600})}catch(e){alert("error during autocomplete setup"+e)}},addAdvancedGroup:function(button){var currentRow;if(button==undefined){currentRow=$(".advancedRow").last()}else{currentRow=$(button).closest(".advancedRow")}var clonedData=currentRow.clone();clonedData.find(".btn").removeClass("active");clonedData.find(".lookfor").val("");clonedData.insertAfter(currentRow);VuFind.Searches.resetAdvancedRowIds();return false},deleteAdvancedGroup:function(button){var currentRow=$(button).closest(".advancedRow");currentRow.remove();VuFind.Searches.resetAdvancedRowIds();return false},sendEmail:function(){if(Globals.loggedIn){var from=$("#from").val();var to=$("#to").val();var message=$("#message").val();var related_record=$("#related_record").val();var sourceUrl=window.location.href;var url=Globals.path+"/Search/AJAX";$.getJSON(url,{method:"sendEmail",from:from,to:to,message:message,sourceUrl:sourceUrl},function(data){if(data.result){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Error",data.message)}})}return false},enableSearchTypes:function(){var searchTypeElement=$("#searchSource");var catalogType="catalog";if(searchTypeElement){var selectedSearchType=$(searchTypeElement.find(":selected"));if(selectedSearchType){catalogType=selectedSearchType.data("catalog_type")}}if(catalogType=="catalog"||catalogType==null){$(".catalogType").show();$(".genealogyType").hide()}else{$(".catalogType").hide();$(".genealogyType").show()}},lastSpellingTimer:undefined,getSpellingSuggestion:function(query,process,isAdvanced){if(VuFind.Searches.lastSpellingTimer!=undefined){clearTimeout(VuFind.Searches.lastSpellingTimer);VuFind.Searches.lastSpellingTimer=undefined}var url=Globals.path+"/Search/AJAX?method=GetAutoSuggestList&searchTerm="+query;if(isAdvanced){}VuFind.Searches.lastSpellingTimer=setTimeout(function(){$.get(url,function(data){process(data)},"json")},500)},loadSearchGroups:function(){var searchGroups=VuFind.Searches.searchGroups;for(var i=0;i<searchGroups.length;i++){if(i>0){VuFind.Searches.addAdvancedGroup()}var searchGroup=searchGroups[i];var groupIndex=i+1;var searchGroupElement=$("#group"+groupIndex);searchGroupElement.find(".groupStartInput").val(searchGroup.groupStart);if(searchGroup.groupStart==1){searchGroupElement.find(".groupStartButton").addClass("active")}searchGroupElement.find(".searchType").val(searchGroup.searchType);searchGroupElement.find(".lookfor").val(searchGroup.lookfor);searchGroupElement.find(".groupEndInput").val(searchGroup.groupEnd);if(searchGroup.groupEnd==1){searchGroupElement.find(".groupEndButton").addClass("active")}searchGroupElement.find(".joinOption").val(searchGroup.join)}if(searchGroups.length==0){VuFind.Searches.resetAdvancedRowIds()}},processSearchForm:function(){var searchSource=$("#searchSource");if(searchSource.val()=="existing"){$(".existingFilter").prop("checked",true);var originalSearchSource=$("#existing_search_option").data("original_type");searchSource.val(originalSearchSource)}},resetAdvancedRowIds:function(){var searchRows=$(".advancedRow");searchRows.each(function(index,element){var indexVal=index+1;var curRow=$(element);curRow.attr("id","group"+indexVal);curRow.find(".groupStartInput").prop("name","groupStart["+indexVal+"]").attr("id","groupStart"+indexVal+"Input");curRow.find(".groupStartButton").data("hidden_element","groupStart"+indexVal+"Input").attr("id","groupStart"+indexVal);curRow.find(".searchType").attr("name","searchType["+indexVal+"]");curRow.find(".lookfor").attr("name","lookfor["+indexVal+"]");curRow.find(".groupEndInput").prop("name","groupEnd["+indexVal+"]").attr("id","groupEnd"+indexVal+"Input");curRow.find(".groupEndButton").data("hidden_element","groupEnd"+indexVal+"Input").attr("id","groupEnd"+indexVal);curRow.find(".joinOption").attr("name","join["+indexVal+"]")});if(searchRows.length==1){$(".deleteCriteria").hide();$(".groupStartButton").hide();$(".groupEndButton").hide()}else{$(".deleteCriteria").show();$(".groupStartButton").show();$(".groupEndButton").show()}var joinOptions=$(".joinOption");joinOptions.show();joinOptions.last().hide()},resetSearchType:function(){if($("#lookfor").val()==""){$("#searchSource").val($("#default_search_type").val())}return true},updateSearchTypes:function(catalogType,searchType,searchFormId){if(catalogType=="catalog"){$("#basicType").val(searchType);$("#genealogyType").remove()}else{$("#genealogyType").val(searchType);$("#basicType").remove()}$(searchFormId).submit();return false},filterAll:function(){$(".existingFilter").prop("checked",true)},submitAdvancedSearch:function(){$("#advancedPopup").submit();return false}}}(VuFind.Searches||{});
function TitleScroller(scrollerId,scrollerShortName,container,autoScroll,style){this.scrollerTitles=[];this.currentScrollerIndex=0;this.numScrollerTitles=0;this.scrollerId=scrollerId;this.scrollerShortName=scrollerShortName;this.container=container;this.scrollInterval=0;this.swipeInterval=5;this.autoScroll=typeof autoScroll=="undefined"?false:autoScroll;this.style=typeof style=="undefined"?"horizontal":style}TitleScroller.prototype.loadTitlesFrom=function(jsonUrl){jsonUrl=decodeURIComponent(jsonUrl);var scroller=this,scrollerBody=$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerBody");scrollerBody.hide();$("#titleScrollerSelectedTitle"+this.scrollerShortName+",#titleScrollerSelectedAuthor"+this.scrollerShortName).html("");$(".scrollerLoadingContainer").show();$.getJSON(jsonUrl,function(data){scroller.loadTitlesFromJsonData(data)}).error(function(){scrollerBody.html("Unable to load titles. Please try again later.").show();$(".scrollerLoadingContainer").hide()})};TitleScroller.prototype.loadTitlesFromJsonData=function(data){var scroller=this,scrollerBody=$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerBody");try{if(data.error)throw{description:data.error};if(data.titles.length==0){scrollerBody.html("No titles were found for this list. Please try again later.");$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerLoadingContainer").hide();scrollerBody.show()}else{scroller.scrollerTitles=[];var i=0;$.each(data.titles,function(key,val){scroller.scrollerTitles[i++]=val});if(scroller.container&&data.titles.length>0){$("#"+scroller.container).fadeIn()}scroller.numScrollerTitles=data.titles.length;if(this.style=="horizontal"||this.style=="vertical"){scroller.currentScrollerIndex=data.currentIndex}else{scroller.currentScrollerIndex=0}TitleScroller.prototype.updateScroller.call(scroller)}}catch(err){if(scrollerBody!=null){scrollerBody.html("error loading titles from data "+err.description+". Please try again later.").show();$(".scrollerLoadingContainer").hide()}}};TitleScroller.prototype.updateScroller=function(){var scrollerBody=$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerBody");try{var scrollerBodyContents="",curScroller=this;if(this.style=="horizontal"){for(var i in this.scrollerTitles){scrollerBodyContents+=this.scrollerTitles[i]["formattedTitle"]}scrollerBody.html(scrollerBodyContents).width(this.scrollerTitles.length*131).waitForImages(function(){TitleScroller.prototype.finishLoadingScroller.call(curScroller)})}else if(this.style=="vertical"){for(var j in this.scrollerTitles){scrollerBodyContents+=this.scrollerTitles[j]["formattedTitle"]}scrollerBody.html(scrollerBodyContents).height(this.scrollerTitles.length*131).waitForImages(function(){TitleScroller.prototype.finishLoadingScroller.call(curScroller)})}else if(this.style=="text-list"){$.each(this.scrollerTitles,function(j,val){scrollerBodyContents=$("<div>").attr({id:curScroller.scrollerId+j.toString(),"class":"scrollerTitle"}).html($("<a><b>"+val["title"]+"</b> by <b>"+val["author"]+"</b></a>").attr({href:val["titleURL"],onclick:"trackEvent('ListWidget', 'Title Click', '"+this.scrollerShortName+"')",id:"descriptionTrigger"+val["shortId"]}).prepend(j+1+") "));scrollerBodyContents.appendTo(scrollerBody)});scrollerBody.height(this.scrollerTitles.length*40);TitleScroller.prototype.finishLoadingScroller.call(curScroller)}else{this.currentScrollerIndex=0;scrollerBody.html(this.scrollerTitles[this.currentScrollerIndex]["formattedTitle"]);TitleScroller.prototype.finishLoadingScroller.call(this)}}catch(err){alert("error in updateScroller for scroller "+this.scrollerId+" "+err.description);scrollerBody.html("error loading titles from data "+err+". Please try again later.").show();$(".scrollerLoadingContainer").hide()}};TitleScroller.prototype.finishLoadingScroller=function(){$(".scrollerLoadingContainer").hide();$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerBody").show();TitleScroller.prototype.activateCurrentTitle.call(this);var curScroller=this;$(".scrollerTitle").bind("mouseover",{scroller:curScroller},function(){curScroller.hovered=true}).bind("mouseout",{scroller:curScroller},function(){curScroller.hovered=false});curScroller.hovered=false;if(this.autoScroll&&this.scrollInterval==0){this.scrollInterval=setInterval(function(){if(!curScroller.hovered){curScroller.scrollToRight()}},5e3)}};TitleScroller.prototype.scrollToRight=function(){this.currentScrollerIndex++;if(this.currentScrollerIndex>this.numScrollerTitles-1)this.currentScrollerIndex=0;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.scrollToLeft=function(){this.currentScrollerIndex--;if(this.currentScrollerIndex<0)this.currentScrollerIndex=this.numScrollerTitles-1;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.swipeToRight=function(customSwipeInterval){customSwipeInterval=typeof customSwipeInterval==="undefined"?this.swipeInterval:customSwipeInterval;this.currentScrollerIndex-=customSwipeInterval;if(this.currentScrollerIndex<0)this.currentScrollerIndex=this.numScrollerTitles-1;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.swipeToLeft=function(customSwipeInterval){customSwipeInterval=typeof customSwipeInterval==="undefined"?this.swipeInterval:customSwipeInterval;this.currentScrollerIndex+=customSwipeInterval;if(this.currentScrollerIndex>this.numScrollerTitles-1)this.currentScrollerIndex=0;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.swipeUp=function(customSwipeInterval){customSwipeInterval=typeof customSwipeInterval==="undefined"?this.swipeInterval:customSwipeInterval;this.currentScrollerIndex-=customSwipeInterval;if(this.currentScrollerIndex<0)this.currentScrollerIndex=this.numScrollerTitles-1;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.swipeDown=function(customSwipeInterval){customSwipeInterval=typeof customSwipeInterval==="undefined"?this.swipeInterval:customSwipeInterval;this.currentScrollerIndex+=customSwipeInterval;if(this.currentScrollerIndex>this.numScrollerTitles-1)this.currentScrollerIndex=0;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.activateCurrentTitle=function(){if(this.numScrollerTitles==0){return}var scrollerTitles=this.scrollerTitles,scrollerShortName=this.scrollerShortName,currentScrollerIndex=this.currentScrollerIndex,scrollerBody=$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerBody"),scrollerTitleId="#scrollerTitle"+this.scrollerShortName+currentScrollerIndex;$("#tooltip").hide();if(this.style=="horizontal"){$("#titleScrollerSelectedTitle"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["title"]);$("#titleScrollerSelectedAuthor"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["author"]);if($(scrollerTitleId).length!=0){var widthItemsLeft=$(scrollerTitleId).position().left,widthCurrent=$(scrollerTitleId).width(),containerWidth=$("#"+this.scrollerId+" .scrollerBodyContainer").width(),leftPosition=-(widthItemsLeft+widthCurrent/2-containerWidth/2);scrollerBody.animate({left:leftPosition+"px"},400,function(){for(var i in scrollerTitles){var scrollerTitleId2="#scrollerTitle"+scrollerShortName+i;$(scrollerTitleId2).removeClass("selected")}$(scrollerTitleId).addClass("selected")})}}else if(this.style=="vertical"){$("#titleScrollerSelectedTitle"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["title"]);$("#titleScrollerSelectedAuthor"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["author"]);if($(scrollerTitleId).length!=0){var relativeTopOfElement=$(scrollerTitleId).position().top,topPosition=25-relativeTopOfElement;scrollerBody.animate({top:topPosition+"px"},400,function(){for(var i in scrollerTitles){var scrollerTitleId2="#scrollerTitle"+scrollerShortName+i;$(scrollerTitleId2).removeClass("selected")}$(scrollerTitleId).addClass("selected")})}}else if(this.style=="text-list"){}else{$("#titleScrollerSelectedTitle"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["title"]);$("#titleScrollerSelectedAuthor"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["author"]);scrollerBody.left="0px";scrollerBody.html(this.scrollerTitles[currentScrollerIndex]["formattedTitle"])}};(function($){$.fn.waitForImages=function(finishedCallback,eachCallback){eachCallback=eachCallback||function(){};if(!$.isFunction(finishedCallback)||!$.isFunction(eachCallback)){throw{name:"invalid_callback",message:"An invalid callback was supplied."}}var objs=$(this),allImgs=objs.find("img"),allImgsLength=allImgs.length,allImgsLoaded=0;if(allImgsLength==0){finishedCallback.call(this)}else{setTimeout(function(){finishedCallback.call(this)},1e4)}return objs.each(function(){var obj=$(this),imgs=obj.find("img");if(imgs.length==0){return}imgs.each(function(){var image=new Image,imgElement=this;image.onload=function(){allImgsLoaded++;eachCallback.call(imgElement,allImgsLoaded,allImgsLength);if(allImgsLoaded==allImgsLength){finishedCallback.call(obj[0])}return false};image.onabort=function(){allImgsLoaded++;eachCallback.call(imgElement,allImgsLoaded,allImgsLength);if(allImgsLoaded==allImgsLength){finishedCallback.call(obj[0])}return false};image.onerror=function(){allImgsLoaded++;eachCallback.call(imgElement,allImgsLoaded,allImgsLength);if(allImgsLoaded==allImgsLength){finishedCallback.call(obj[0])}return false};image.src=this.src})})}})(jQuery);
