...
VuFind.Browse=function(){return{curPage:1,curCategory:"",addToHomePage:function(searchId){VuFind.Account.ajaxLightbox(Globals.path+"/Browse/AJAX?method=getAddBrowseCategoryForm&searchId="+searchId,true);return false},initializeBrowseCategory:function(){var browseCategoryCarousel=$("#browse-category-carousel");browseCategoryCarousel.on("jcarousel:create jcarousel:reload",function(){var Carousel=$(this),width=Carousel.innerWidth();if(width>700){width=width/4}else if(width>500){width=width/3}else if(width>400){width=width/2}Carousel.jcarousel("items").css("width",Math.floor(width)+"px")});browseCategoryCarousel.on("jcarousel:targetin","li",function(){var categoryId=$(this).data("category-id");VuFind.Browse.changeBrowseCategory(categoryId)});if($("#browse-category-picker .jcarousel-control-prev").css("display")!="none"){browseCategoryCarousel.find("li").click(function(){$("#browse-category-carousel").jcarousel("scroll",$(this))});var scrollFactor=15;browseCategoryCarousel.touchwipe({wipeLeft:function(dx){var scrollInterval=Math.round(dx/scrollFactor);$("#browse-category-carousel").jcarousel("scroll","+="+scrollInterval)},wipeRight:function(dx){var scrollInterval=Math.round(dx/scrollFactor);$("#browse-category-carousel").jcarousel("scroll","-="+scrollInterval)}})}else{browseCategoryCarousel.find("li").click(function(){$(this).trigger("jcarousel:targetin")})}},changeBrowseCategory:function(categoryTextId){var url=Globals.path+"/Browse/AJAX",params={method:"getBrowseCategoryInfo",textId:categoryTextId},newLabel=$("#browse-category-"+categoryTextId+" div").first().text();$(".browse-category").removeClass("selected");$("#browse-category-"+categoryTextId).addClass("selected");$(".selected-browse-label-search-text").fadeOut(function(){$(this).html(newLabel).attr("href","").fadeIn()});$(".home-page-browse-thumbnails, .home-page-browse-thumbnails ~ hr").fadeOut(function(){$(".home-page-browse-thumbnails, .home-page-browse-thumbnails ~ hr").slice(1).remove()});if(VuFind.Browse.reload)params.reload="";$.getJSON(url,params,function(data){if(data.result==false){VuFind.showMessage("Error loading browse information","Sorry, we were not able to find titles for that category")}else{if(data.label!=newLabel)$(".selected-browse-label-search-text").html(data.label);VuFind.Browse.curPage=1;VuFind.Browse.curCategory=data.textId;$(".home-page-browse-thumbnails").html(data.records).fadeIn("slow");$("#selected-browse-search-link").attr("href",data.searchUrl)}}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.");$(".home-page-browse-thumbnails").html("").show()});return false},createBrowseCategory:function(){var url=Globals.path+"/Browse/AJAX?method=createBrowseCategory";url+="&searchId="+$("#searchId").val();url+="&categoryName="+$("#categoryName").val();$.getJSON(url,function(data){if(data.result==false){VuFind.showMessage("Unable to create category",data.message)}else{VuFind.showMessage("Successfully added","This search was added to the homepage successfully.",true)}});return false},getMoreResults:function(){var url=Globals.path+"/Browse/AJAX",params={method:"getMoreBrowseResults",textId:this.curCategory,pageToLoad:this.curPage+1};$.getJSON(url,params,function(data){if(data.result==false){VuFind.showMessage("Error loading browse information","Sorry, we were not able to find titles for that category")}else{var newDiv=$('<div class="home-page-browse-thumbnails row" />').hide().append(data.records);$(".home-page-browse-thumbnails").filter(":last").after(newDiv).after("<hr>");newDiv.fadeIn("slow");VuFind.Browse.curPage++}}).fail(function(){VuFind.showMessage("Request Failed","There was an error with this AJAX Request.")});return false}}}(VuFind.Browse||{});
...
VuFind.Ratings=function(){$(document).ready(function(){VuFind.Ratings.initializeRaters()});return{initializeRaters:function(){$(".rater").each(function(){var ratingElement=$(this);var module=ratingElement.data("module");var userRating=ratingElement.data("user_rating");var recordId=ratingElement.data("record_id");var shortId=ratingElement.data("short_id");var id=ratingElement.data("id");if(shortId==null){shortId=id}if(recordId==null){recordId=id}var options={module:module,recordId:shortId,rating:parseFloat(userRating>0?userRating:ratingElement.data("average_rating")),postHref:Globals.path+module+"/"+recordId+"/AJAX?method=RateTitle"};ratingElement.rater(options)})},doRatingReview:function(rating,module,id){if(rating<=2){msg="We're sorry you didn't like this title.  Would you like to add a review explaining why to help other users?"}else{msg="We're glad you liked this title.  Would you like to add a review explaining why to help other users?"}if(confirm(msg)){var reviewForm;if(module=="EcontentRecord"){reviewForm=$("#userecontentreview"+id)}else{reviewForm=$("#userreview"+id)}reviewForm.find(".rateTitle").hide();reviewForm.show()}}}}(VuFind.Ratings);
