...
VuFind.Menu=function(){$(function(){$(".menu-icon").click(function(){$(".menu-icon").removeClass("menu-icon-selected");$(this).addClass("menu-icon-selected")});var mobileMenu=$("#horizontal-menu-bar-container"),switchPosition=mobileMenu.offset().top;$(window).scroll(function(){var fixedOffset=mobileMenu.offset().top,notFixedScrolledPosition=$(this).scrollTop();if(mobileMenu.is(".sticky-menu-bar")&&fixedOffset<=switchPosition){mobileMenu.removeClass("sticky-menu-bar")}if(!mobileMenu.is(".sticky-menu-bar")&&notFixedScrolledPosition>=switchPosition){mobileMenu.addClass("sticky-menu-bar")}})});return{temp:function(){alert("Click");return false},hideAll:function(){return $("#home-page-search,#horizontal-search-container,#home-account-links,#home-page-library-section").filter(":visible").slideUp()},showMenuSection:function(sectionSelector){$.when(this.hideAll()).done(function(){$(sectionSelector).slideDown()})},showSearch:function(){this.showMenuSection("#home-page-search,#horizontal-search-container")},showMenu:function(){this.showMenuSection("#home-page-library-section")},showAccount:function(){this.showMenuSection("#home-account-links")}}}(VuFind.Menu||{});
...
VuFind.Responsive=function(){$(document).ready(function(){$("#lookfor","#home-page-search").on("keyup",function(event){$(this).height(0);if(this.scrollHeight<32){$(this).height(18)}else{$(this).height(this.scrollHeight)}}).keyup();$("#lookfor").on("keydown",function(event){if(event.which==13||event.which==10){event.preventDefault();event.stopPropagation();$("#searchForm").submit();return false}}).on("keypress",function(event){if(event.which==13||event.which==10){event.preventDefault();event.stopPropagation();return false}})});try{var mediaQueryList=window.matchMedia("print");mediaQueryList.addListener(function(mql){VuFind.Responsive.isPrint=mql.matches;VuFind.Responsive.adjustLayout()})}catch(err){}window.onbeforeprint=function(){VuFind.Responsive.isPrint=true;VuFind.Responsive.adjustLayout()};return{adjustLayout:function(){var resolution=document.documentElement.clientWidth;var mainContentElement=$("#main-content-with-sidebar");var xsContentInsertionPointElement=$("#xs-main-content-insertion-point");var mainContent;if(resolution<768&&!VuFind.Responsive.isPrint){mainContent=mainContentElement.html();if(mainContent&&mainContent.length){xsContentInsertionPointElement.html(mainContent);mainContentElement.html("");VuFind.initCarousels()}}else{mainContent=xsContentInsertionPointElement.html();if(mainContent&&mainContent.length){mainContentElement.html(mainContent);xsContentInsertionPointElement.html("");VuFind.initCarousels()}}}}}(VuFind.Responsive||{});
