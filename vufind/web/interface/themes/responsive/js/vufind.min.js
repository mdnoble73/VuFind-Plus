...
VuFind.Responsive=function(){$(function(){$(window).resize(function(){VuFind.Responsive.adjustLayout()}).trigger("resize");$().ready(function(){VuFind.Responsive.adjustLayout()});$("#lookfor","#home-page-search").on("keyup",function(event){$(this).height(0);if(this.scrollHeight<32){$(this).height(18)}else{$(this).height(this.scrollHeight)}}).keyup();$("#lookfor").on("keydown",function(event){if(event.which==13||event.which==10){event.preventDefault();event.stopPropagation();$("#searchForm").submit();return false}}).on("keypress",function(event){if(event.which==13||event.which==10){event.preventDefault();event.stopPropagation();return false}})});try{var mediaQueryList=window.matchMedia("print");mediaQueryList.addListener(function(mql){VuFind.Responsive.isPrint=mql.matches})}catch(err){}window.onbeforeprint=function(){VuFind.Responsive.isPrint=true};return{adjustLayout:function(){var resolutionX=document.documentElement.clientWidth;if(resolutionX>=768&&!VuFind.Responsive.isPrint){var mainContentElement=$("#main-content-with-sidebar");var sidebarContentElem=$("#sidebar-content");var maxHeight=Math.max(mainContentElement.height()+15,sidebarContentElem.height());if(mainContentElement.height()+15<maxHeight){mainContentElement.height(maxHeight)}if(sidebarContentElem.height()<maxHeight){sidebarContentElem.height(maxHeight)}}}}}(VuFind.Responsive||{});
