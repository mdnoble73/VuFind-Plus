...
VuFind.Archive=function(){return{initializeOpenSeadragon:function(viewer){viewer.addHandler("open",this.update_clip);viewer.addHandler("animationfinish",this.update_clip)},update_clip:function(viewer){var fitWithinBoundingBox=function(d,max){if(d.width/d.height>max.x/max.y){return new OpenSeadragon.Point(max.x,parseInt(d.height*max.x/d.width))}else{return new OpenSeadragon.Point(parseInt(d.width*max.y/d.height),max.y)}};var getDisplayRegion=function(viewer,source){var box=new OpenSeadragon.Rect(0,0,source.x,source.y);var container=viewer.viewport.getContainerSize();var bounds=viewer.viewport.getBounds();if(bounds.x>0){box.x=box.x-viewer.viewport.pixelFromPoint(new OpenSeadragon.Point(0,0)).x}if(box.x+source.x>container.x){box.width=container.x-viewer.viewport.pixelFromPoint(new OpenSeadragon.Point(0,0)).x;if(box.width>container.x){box.width=container.x}}if(bounds.y>0){box.y=box.y-viewer.viewport.pixelFromPoint(new OpenSeadragon.Point(0,0)).y}if(box.y+source.y>container.y){box.height=container.y-viewer.viewport.pixelFromPoint(new OpenSeadragon.Point(0,0)).y;if(box.height>container.y){box.height=container.y}}return box};var source=viewer.source;var zoom=viewer.viewport.getZoom();var size=new OpenSeadragon.Rect(0,0,source.dimensions.x,source.dimensions.y);var container=viewer.viewport.getContainerSize();var fit_source=fitWithinBoundingBox(size,container);var total_zoom=fit_source.x/source.dimensions.x;var container_zoom=fit_source.x/container.x;var level=zoom*total_zoom/container_zoom;var box=getDisplayRegion(viewer,new OpenSeadragon.Point(parseInt(source.dimensions.x*level),parseInt(source.dimensions.y*level)));var scaled_box=new OpenSeadragon.Rect(parseInt(box.x/level),parseInt(box.y/level),parseInt(box.width/level),parseInt(box.height/level));var params={url_ver:"Z39.88-2004",rft_id:source.imageID,svc_id:"info:lanl-repo/svc/getRegion",svc_val_fmt:"info:ofi/fmt:kev:mtx:jpeg2000","svc.format":"image/jpeg","svc.region":scaled_box.y+","+scaled_box.x+","+(scaled_box.getBottomRight().y-scaled_box.y)+","+(scaled_box.getBottomRight().x-scaled_box.x)};var dimensions=zoom<=1?source.dimensions.x+","+source.dimensions.y:container.x+","+container.y;jQuery("#clip").attr("href","https://islandora.marmot.org/islandora/object/"+settings.islandoraOpenSeadragon.pid+"/print?"+jQuery.param({clip:source.baseURL+"?"+jQuery.param(params),dimensions:dimensions}))}}}(VuFind.Archive||{});
...
VuFind.Menu=function(){$(function(){VuFind.Menu.AllSideBarSelectors=VuFind.Menu.SideBarSearchSelectors+","+VuFind.Menu.SideBarAccountSelectors+","+VuFind.Menu.SideBarMenuSelectors;VuFind.Menu.stickyMenu("#horizontal-menu-bar-container","sticky-menu-bar");VuFind.Menu.stickyMenu("#vertical-menu-bar","sticky-sidebar");if($("#horizontal-menu-bar-container").is(":visible")){VuFind.Menu.hideAllFast();$("#refineSearch").hide()}});return{SideBarSearchSelectors:"#home-page-search,#horizontal-search-container,#narrow-search-label,#facet-accordion,#results-sort-label,#results-sort-label+div.row,#remove-search-label,#remove-search-label+.applied-filters,#similar-authors",SideBarAccountSelectors:"#home-page-login,#home-account-links",SideBarMenuSelectors:"#home-page-login,#home-page-library-section",ExploreMoreSelectors:"",AllSideBarSelectors:"",stickyMenu:function(menuContainerSelector,stickyMenuClass){var menu=$(menuContainerSelector),viewportHeight=$(window).height(),switchPosition;if(menu.is(":visible"))switchPosition=menu.offset().top;$(window).resize(function(){viewportHeight=$(this).height()});$(window).scroll(function(){if(menu.is(":visible")&&viewportHeight<$("#main-content-with-sidebar").height()){if(typeof switchPosition=="undefined"){switchPosition=menu.offset().top}var fixedOffset=menu.offset().top,notFixedScrolledPosition=$(this).scrollTop();if(menu.is("."+stickyMenuClass)&&fixedOffset<=switchPosition){menu.removeClass(stickyMenuClass)}if(!menu.is("."+stickyMenuClass)&&notFixedScrolledPosition>=switchPosition){menu.addClass(stickyMenuClass)}}})},hideAll:function(){return $(VuFind.Menu.AllSideBarSelectors).filter(":visible").slideUp()},hideAllFast:function(){return $(VuFind.Menu.AllSideBarSelectors).filter(":visible").hide()},collapseSideBar:function(){if($(VuFind.Menu.AllSideBarSelectors).filter(":visible").length==0){$("#side-bar,#vertical-menu-bar-container").addClass("collapsedSidebar");$("#main-content-with-sidebar").addClass("mainContentWhenSiderbarCollapsed")}},openSideBar:function(){$("#main-content-with-sidebar").removeClass("mainContentWhenSiderbarCollapsed");$("#side-bar,#vertical-menu-bar-container").removeClass("collapsedSidebar")},showMenuSection:function(sectionSelector,clickedElement){$.when(this.hideAll()).done(function(){var elem=$(clickedElement),parent=elem.parent(".menu-bar-option");if(parent.length>0){if(parent.is(".menu-icon-selected")){parent.removeClass("menu-icon-selected");VuFind.Menu.collapseSideBar()}else{$(".menu-bar-option").removeClass("menu-icon-selected");parent.addClass("menu-icon-selected");VuFind.Menu.openSideBar();$(sectionSelector).slideDown()}}else{VuFind.Menu.openSideBar();if(elem.is(".menu-icon-selected")){elem.removeClass("menu-icon-selected");$(sectionSelector).slideUp()}else{$(".menu-icon-selected","#horizontal-menu-bar-container").removeClass("menu-icon-selected");elem.addClass("menu-icon-selected");$(sectionSelector).slideDown()}}})},showSearch:function(clickedElement){var parent=$(clickedElement).parent(".menu-bar-option");if(parent&&!parent.is(".menu-icon-selected")&&$("#home-page-search,#narrow-search-label,#facet-accordion,#results-sort-label").length==0){$.when(this.hideAll()).done(function(){VuFind.Menu.collapseSideBar();$(".menu-bar-option").removeClass("menu-icon-selected");parent.addClass("menu-icon-selected");$("#horizontal-search-container").slideDown()})}else{this.showMenuSection(this.SideBarSearchSelectors,clickedElement)}},showMenu:function(clickedElement){this.showMenuSection(this.SideBarMenuSelectors,clickedElement)},showAccount:function(clickedElement){this.showMenuSection(this.SideBarAccountSelectors,clickedElement)},showExploreMore:function(clickedElement){this.showMenuSection(this.ExploreMoreSelectors,clickedElement)},showSearchFacets:function(){$("#refineSearch").toggle();var btn=$("#refineSearchButton");btn.text(btn.text()=="Refine Search"?"Hide Refine Search":"Refine Search")}}}(VuFind.Menu||{});
