			...
			{foreach from=$libraryLinks item=linkCategory key=categoryName name=linkLoop}
				{if $categoryName}
					...
					<div class="panel {if $smarty.foreach.linkLoop.first && !$user}active{/if}">
						...
						<div id="{$categoryName|escapeCSS}Panel" class="panel-collapse collapse {if ($smarty.foreach.linkLoop.first && !$user) || array_key_exists($categoryName, $expandedLinkCategories)}in{/if}">
