...
<ul class="similar">
  ...
  <li class="prospectorTitle {if $prospectorTitle.isCurrent}currentRecord{/if}">
    <a href="{$prospectorTitle.link}">{$prospectorTitle.title|regex_replace:"/(\/|:)$/":""|escape}</a>
...
<div id='prospectorAvailability'>
  {if $prospectorDetails.prospectorEncoreUrl}<a href="{$prospectorDetails.prospectorEncoreUrl}">{/if}Available in Prospector{if $prospectorTitle.link}</a>{/if}
  <span class='prospectorRequest'><a class='holdRequest' href='#' onclick="createInnreachRequestWindow('{$prospectorDetails.requestUrl}')">Request from Prospector</a></span>
</div>
