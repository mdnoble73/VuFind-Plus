	...
	<?php if ($page == 'overview'): ?>
		...
		<div class="opc-container">
			<div id="counts">
				...
				<div class="values">
					<p><b>total memory:</b> <span data-value="total_memory_size"><?php echo $data['total_memory_size']; ?></span>
					</p>
					...
					<p><b>wasted memory:</b> <span class="realtime" data-value="wasted_memory_size"><?php echo $data['wasted_memory_size']; ?></span>
						(<span class="realtime" data-value="wasted_percentage"><?php echo $data['wasted_percentage']; ?></span>%)
					</p>

					<p><b>number of cached files:</b> <span class="realtime" data-value="files_cached"><?php echo $data['files_cached']; ?></span>
					</p>
					...
					<p><b>blacklist misses:</b> <span class="realtime" data-value="blacklist_miss_size"><?php echo $data['blacklist_miss_size']; ?></span>
					</p>

					<p><b>number of cached keys:</b> <span class="realtime" data-value="num_cached_keys_size"><?php echo $data['num_cached_keys_size']; ?></span>
					</p>
			...
			<div id="info">
				...
				<table>
					...
					<?php foreach ($opcache_config['directives'] as $d => $v): ?>
						<tr>
							<td><span
									title="<?php echo $d; ?>"><?php echo str_replace(array('opcache.', '_'), array('', ' '), $d); ?></span>
							</td>
				...
				<table>
					...
					<?php foreach ($opcache_funcs as $f): ?>
						<tr>
							<td><a href="http://php.net/<?php echo $f; ?>" title="View manual page" target="_blank"><?php echo $f; ?>
							</td>
						</tr>
					<?php endforeach; ?>
				</table>
	...
	<?php if ($page == 'files'): ?>
		...
		<div class="opc-container">
			...
			<table>
				...
				<?php foreach ($opcache_status['scripts'] as $s): ?>
					<tr>
		...
		<script type="text/javascript">
			$(function () {
				...
				$('.opc-container table').bind('paint', function (event, params) {
				...
				$('#frmFilter').bind('keyup', function (event) {
					...
					$('.opc-container table').trigger('paint');
				});
			});
		</script>
	<?php endif;
