...
class DeletedEContent extends Admin_Admin
{
	function launch()
	{
		global $interface;
		...
		$interface->setPageTitle('Deleted eContent');
		...
		$query = "SELECT econtent_record.id, title, author, isbn, ilsId, source, date_updated FROM econtent_record WHERE status = 'deleted' $dateRestriction $sourceRestriction";
		$eContentRecord->query($query);
		
		$deletedRecords = array();
		while ($eContentRecord->fetch()){
			$deletedRecords[] = clone($eContentRecord);
		}
		$interface->assign('deletedRecords', $deletedRecords);
	...
	function exportToExcel($itemlessRecords){
		...
		$objPHPExcel->getProperties()->setCreator("DCL")
			...
			->setCategory("Deleted eContent Report");

		// Add some data
		$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue('A1', 'Deleted eContent')
			->setCellValue('A3', 'ID')
		...
		header('Content-Disposition: attachment;filename=DeletedEContentReport.xls');
