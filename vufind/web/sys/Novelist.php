...
require_once 'Drivers/marmot_inc/NovelistCache.php';
...
class Novelist{
    ...
    function loadEnrichment($isbn, $loadSeries = true, $loadSimilarTitles = true, $loadSimilarAuthors = true){
        ...
        $novelistCache = new NovelistCache();
        $cacheExpirationTime = time() - (7 * 24 * 60 * 60);
        $novelistCache->whereAdd("cacheDate < $cacheExpirationTime");
        $novelistCache->delete(true);
        
        $novelistCache = new NovelistCache();
        $novelistCache->isbn = $isbn;
        $novelistCache->find();
        if ($novelistCache->N > 0){
            //Found a cache entry
            $novelistCache->fetch();
            $enrichment = json_decode($novelistCache->enrichmentInfo, TRUE);
            return $enrichment;
        }
        ...
        try{
	        $novelistCache->isbn = $isbn;
	        $json_data = json_encode($enrichment);
	        $novelistCache->enrichmentInfo = $json_data;
	        $novelistCache->cacheDate = time();
	        $novelistCache->insert();
	        $timer->logTime("Cached enrichment information");
        }catch(Exception $e){
        	$logger = new Logger();
            $logger->log("Error saving NoveList information to cache $e", PEAR_LOG_ERROR);
            //$enrichment = null;
        }
